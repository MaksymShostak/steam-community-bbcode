...
const pythonEnvironment = {
  ...process.env,
  PYTHONPATH: local("artifacts/comparison/python"),
  PYTHONIOENCODING: "utf-8",
  PYTHONNOUSERSITE: "1",
};
const dotnetPackage =
  "artifacts/comparison/dotnet/.store/converter.markdowntobbcodesteam.tool/1.0.0.29/converter.markdowntobbcodesteam.tool/1.0.0.29/";
const syntax = gfmFromMarkdown().map(
  ({ transforms: _rendererTransforms, ...extension }) => extension,
);
/** @param {string} source */
const parseGfm = (source) =>
  fromMarkdown(source, { extensions: [gfm()], mdastExtensions: syntax });
...
async function convert(provider, fixture) {
  if (provider === "candidate") {
    const result =
      fixture.direction === "forward"
        ? steamCommunityBbcodeToGfm(fixture.source)
        : gfmToSteamCommunityBbcode(fixture.source);
    return {
      output: result.value,
      diagnostics: result.diagnostics
        .filter((diagnostic) =>
          ["approximate", "lossy", "unsupported"].includes(diagnostic.fidelity),
        )
        .map((diagnostic) => diagnostic.code),
      stderr: "",
    };
  }
  if (provider === "steamify" || provider === "steam-editor-tools")
    return execute(
      python,
      [
        "-P",
        local("comparison/python/convert.py"),
        `${provider}-${fixture.direction}`,
      ],
      fixture.source,
      pythonEnvironment,
    );
  ...
  return execute(
    local("artifacts/comparison/dotnet/markdown_to_bbcodesteam.exe"),
    ["-i", input],
    "",
  );
}
...
for (const provider of providerIds) {
  for (const fixture of comparisonCases) {
    ...
    if (capability.applicable) {
      try {
        ...
        if (execution.error === undefined) {
          if (fixture.direction === "forward")
            actualTree = semanticProjection(parseGfm(execution.output));
          else {
            const consumed = steamCommunityBbcodeToGfm(execution.output);
            actualTree = semanticProjection(parseGfm(consumed.value));
            consumerDiagnostics = consumed.diagnostics.map(
    ...
    const status = classifyResult({
      ...
      actualTree,
      requiresDiagnosis: fixture.requiresDiagnosis,
    ...
    results.push({
      ...
      status,
      execution,
      actualTree: actualTree ?? null,
      consumerDiagnostics,
    });
  }
  process.stdout.write(
    `${provider}: ${JSON.stringify(countStatuses(results.filter((result) => result.provider === provider)))}\n`,
  );
}
...
const inputPaths = [
  "package.json",
  "package-lock.json",
  "scripts/compare-alternatives.js",
  "comparison/python/convert.py",
  "comparison/python/requirements-windows-py314.txt",
  "comparison/dotnet/NuGet.Config",
];
for (const directory of [
  ...
  for (const path of await readdir(local(directory), { recursive: true })) {
    if (/\.(js|json)$/u.test(path))
      inputPaths.push(`${directory}/${path.replaceAll("\\", "/")}`);
  }
}
...
for (const path of inputPaths.sort())
  inputs[path] = createHash("sha256")
    .update((await readFile(local(path), "utf8")).replace(/\r\n?/gu, "\n"))
    .digest("hex");
/** @param {string} path */
...
for (const path of nativeIdentityPaths)
  nativeIdentities[path] = await sha256(path);
const report = {
  schemaVersion: 1,
  license: "AGPL-3.0-only",
  executedAt: new Date().toISOString(),
  scope:
    "Windows x64 Python 3.14 local execution; unmodified public APIs. Selected common cases, not complete project coverage or live Steam rendering.",
  oracle:
    "Authored conformance trees and Steam spellings. Native GFM syntax parser; reverse outputs use the candidate Steam consumer, whose gaps can affect external classifications. Review raw outputs before asserting a comparator defect.",
  interpretation:
    "Exact/equivalent are equally successful semantics. PASS_DIAGNOSED_LOSS requires expected fallback semantics and a per-input diagnostic. FAIL statuses mean mismatch with this corpus contract, not necessarily an upstream bug. No weighted score or global superiority claim.",
  environment: {
    platform: process.platform,
    architecture: process.arch,
    node: process.version,
    python: execute(python, ["--version"], "", pythonEnvironment),
    dotnetRuntimes: execute("dotnet", ["--list-runtimes"], ""),
  },
  versions: {
    candidate: "1.0.0 private development package",
    steamify: "2.0.1",
    "steam-editor-tools": "0.5.1",
    butr: "1.0.0.29",
  },
  inputs,
  canonicalLfInputsSha256: createHash("sha256")
  ...
  corpus: comparisonCases,
  results,
  summary: providerIds.map((provider) => ({
    provider,
    forward: countStatuses(
      results.filter(
        (result) =>
          result.provider === provider && result.direction === "forward",
      ),
    ),
    reverse: countStatuses(
      results.filter(
        (result) =>
          result.provider === provider && result.direction === "reverse",
      ),
    ),
  })),
  historicalSteamifyObligations: {
    fixture: "test/fixtures/steamify/historical-results.json",
    sha256: await sha256("test/fixtures/steamify/historical-results.json"),
    descriptionChecks: 16,
    probeChecks: 17,
    qualification:
      "Existing steamify-description-regression and steamify-probes-regression tests independently exercise all 33 obligations against the candidate. They run through npm test; archived comparator outputs are not relabelled as fresh executions here.",
  },
};
await writeFile(
  local("comparison/results/windows-x64.json"),
...
assert.ok(
  results
    .filter((result) => result.provider === "candidate")
    .every((result) => result.status.startsWith("PASS_")),
