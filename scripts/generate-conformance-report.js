...
const reverseReport = {
  ...
  targetProfile: "workshop-item",
  evidenceScope:
    "Partial GFM syntax subset, independently executed against authored Steam syntax and semantic expectations; not live Steam rendering.",
  executedCaseCount: reverseExecutions.length,
  sourceNodeTypes: [
    ...new Set(reverseExecutions.flatMap((result) => result.sourceNodeTypes)),
  ].sort(),
  executions: reverseExecutions,
};
const contextOnly = steamConstructDefinitions.filter(
  (d) => "observation" in d && d.observation === "contextOnly",
);
const sourceIds = steamConstructDefinitions
  .filter((d) => !contextOnly.some((c) => c.id === d.id))
  .map((d) => d.id);
assert.deepEqual(
  new Set(executions.flatMap((result) => result.constructIds)),
  new Set(sourceIds),
  "Registry and executed cases must have identical source-construct coverage.",
);
...
for (const profile of steamDialectProfileIds) {
  for (const source of [
    "Plain text",
    "[b]A[/b]B",
    "[noparse][b]A[/b][/noparse]",
  ]) {
    const result = steamCommunityBbcodeToGfm(source, { profile });
    for (const definition of contextOnly) {
      const policy = result.coverage.contextOnlyPolicies.find(
        (p) => p.constructId === definition.id,
      );
      assert.ok(
        policy &&
          policy.policy === "preserve-source" &&
          policy.fidelity === "unsupported" &&
          policy.reason,
      );
      assert.ok(
        !result.coverage.constructs.some(
          (c) => c.constructId === definition.id,
        ),
      );
    }
  }
...
const rows = steamConstructDefinitions.map((definition) => {
  const cases = executions.filter((result) =>
    result.constructIds.some((id) => id === definition.id),
  );
  return {
    constructId: definition.id,
    syntaxKind: definition.syntaxKind,
    profiles: definition.profiles,
    provenance: definition.provenance,
    declaredMapping: definition.gfmMapping,
    ...("notes" in definition ? { notes: definition.notes } : {}),
    ...("observation" in definition && definition.observation === "contextOnly"
      ? {
          observation: "contextOnly",
          contextReason: definition.contextReason,
          tests: {
            policy: "pass",
            sourceSyntax: "not-applicable",
            executedContexts: 3 * steamDialectProfileIds.length,
          },
          observedFidelities: ["unsupported"],
        }
      : {
          ...
          observedFidelities: [
            ...new Set(cases.map((result) => result.fidelity)),
          ],
        }),
  };
});
...
const files = [
  "package.json",
  "package-lock.json",
  "scripts/generate-conformance-report.js",
];
for (const directory of ["src", "spec", "test/conformance"]) {
  for (const name of await readdir(
    ...
    if (name.endsWith(".js") || name.endsWith(".json"))
      files.push(`${directory}/${name.replaceAll("\\", "/")}`);
  }
}
...
const report = {
  ...
  evidenceScope:
    "Executed conversion policies; Steam rendering and release readiness are separate qualifications.",
  targetContract:
    "GFM syntax; additional GitHub autolinking is checked separately and diagnosed per input.",
  canonicalLfInputsSha256: digest.digest("hex"),
  registryConstructCount: rows.length,
  sourceConstructCount: sourceIds.length,
  contextOnlyConstructCount: contextOnly.length,
  executedCaseProfiles: executions.length,
  profileCount: steamDialectProfileIds.length,
  rows,
  executions,
...
const document = {
  ...
  children: [
    ...
    },
    {
      type: "heading",
      depth: 1,
      children: [{ type: "text", value: "Steam conversion policy matrices" }],
    },
    {
      type: "paragraph",
      children: [
      ...
      children: [
        {
          ...
          children: [
            "Construct",
            "Policy tests",
            "Observed target fidelity",
            "Steam profile evidence",
          ].map(cell),
        },
        ...rows.map((row) => ({
          type: /** @type {const} */ ("tableRow"),
          children: [
            row.constructId,
            row.observation === "contextOnly"
              ? "Context policy passed; source syntax not observable"
              : "Canonical, nested, escaping, malformed passed",
            row.observedFidelities.join(", "),
    ...
    },
    {
      type: "heading",
      depth: 2,
      children: [{ type: "text", value: "Partial GFM-to-Steam" }],
    },
    {
      type: "paragraph",
      children: [
    ...
    },
    {
      ...
      children: [
        {
          type: "tableRow",
          children: ["Case", "Policy", "Observed fidelity", "Result"].map(cell),
        },
        ...reverseExecutions.map((result) => ({
          type: /** @type {const} */ ("tableRow"),
          children: [
            result.caseId,
            result.policy,
      ...
      children: [
        {
          type: "text",
          value:
            "Machine-readable provenance, exact case identities, input fingerprint and independent reverse results are in ",
        },
        {
          type: "link",
          url: "../coverage.json",
          children: [{ type: "text", value: "coverage.json" }],
        },
        {
          type: "text",
          value:
...
const examples = {
  ...
  children: [
    ...
    },
    {
      type: "heading",
      depth: 1,
      children: [{ type: "text", value: "Conversion semantics by construct" }],
    },
    {
      type: "paragraph",
      children: [
    ...
    },
    {
      ...
      children: [
        ...
        {
          type: "link",
          url: "steam-support-matrix.md",
          children: [{ type: "text", value: "generated support matrix" }],
        },
        {
          type: "text",
          value:
            " for profile provenance, executed contexts and partial reverse policies.",
        },
      ],
    },
  ],
...
for (const definition of steamConstructDefinitions) {
  examples.children.push({
    type: "heading",
    depth: 2,
    children: [{ type: "text", value: definition.id }],
  });
  ...
  examples.children.push(
    ...
    {
      type: "paragraph",
      children: [{ type: "text", value: "Generated GFM:" }],
    },
...
for (const [path, content] of [
  ...
  [
    ...
    toMarkdown(document, { extensions: [gfmToMarkdown()] }),
  ],
...
console.log(
  `${rows.length} registry policies accounted for by ${executions.length} source case/profile executions and ${contextOnly.length} separately checked context-only policies.`,
);
console.log(
  `${reverseReport.executedCaseCount} independent reverse cases cover ${reverseReport.sourceNodeTypes.length} native GFM node kinds with explicit subset policies.`,
);
