...
import {
  githubRenderingScenarios,
  normalizeGithubRenderedHtml,
} from "./github-rendering-scenarios.js";
...
if (!values.live)
  throw new Error(
    "This opt-in qualification contacts GitHub; invoke with --live to execute it.",
  );
const scenarios = githubRenderingScenarios.filter(
  (scenario) => values.scenario === "all" || scenario.name === values.scenario,
);
...
await mkdir(new URL("../artifacts/github-rendering/", import.meta.url), {
  recursive: true,
});
...
for (const scenario of scenarios) {
  ...
  if (scenario.source)
    await writeFile(
      new URL("source.bbcode", directory),
      scenario.source,
      "utf8",
    );
  await writeFile(
    new URL("expected.html", directory),
    scenario.expectedHtml + "\n",
    "utf8",
  );
  const html = await renderMarkdown(scenario.markdown, directory);
  assert.equal(
    normalizeGithubRenderedHtml(html),
    normalizeGithubRenderedHtml(scenario.expectedHtml),
    `${scenario.name}: rendered HTML must preserve the independently specified target.`,
  );
  if (scenario.expectedMarkdown) {
    const expectedHtml = await renderMarkdown(
      scenario.expectedMarkdown,
      directory,
      "reference-",
    );
    assert.equal(
      normalizeGithubRenderedHtml(html),
      normalizeGithubRenderedHtml(expectedHtml),
      "Converted flow must render like the independently authored target, including tight-list structure and protected spaces.",
    );
  }
...
async function renderMarkdown(markdown, directory, prefix = "") {
  ...
  const response = await fetch("https://api.github.com/markdown", {
    ...
    headers: {
      ...
      "User-Agent": "steam-community-bbcode-qualification",
    },
  ...
  await writeFile(
    new URL(`${prefix}provenance.json`, directory),
    JSON.stringify(
      {
        checkedAt,
  ...
  if (!response.ok)
    throw new Error(
      `GitHub Markdown rendering returned HTTP ${response.status}: ${html.slice(0, 300)}`,
    );
  return html;
}
