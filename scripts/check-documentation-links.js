...
export async function checkDocumentationLinks(packageUrl) {
  ...
  const paths = [...(await readdir(packageUrl)).filter(path => path.endsWith('.md')),
    ...docs.map(path => path.replaceAll('\\', '/')).filter(path => path.endsWith('.md')
      && !path.startsWith('plans/') && !path.startsWith('migration/')).map(path => `docs/${path}`)];
  let checked = 0;
  for (const path of paths) {
    ...
    while (pending.length) {
      ...
      assert.ok(destination.href.startsWith(packageUrl.href), `${path}: link escapes package: ${node.url}`);
