...
if (subject === undefined) {
  ...
  assert.ok(
    values.integrity,
    "Public consumers must be bound to the retained archive integrity.",
  );
  for (const credential of ["NPM_TOKEN", "NODE_AUTH_TOKEN"]) {
    assert.ok(
      !process.env[credential],
      "Public registry qualification must run without npm publication credentials.",
    );
  }
}
...
try {
  await writeFile(
    ...
    JSON.stringify({
      name: "steam-community-bbcode-release-consumer",
      version: "0.0.0",
      private: true,
      type: "module",
    }),
  );
  const registryArgs = values.coordinate
    ? [`--registry=${registry}`, "--cache", join(consumer, "cache")]
    : [];
  run(
    [
      npmCli,
  ...
  if (values.coordinate) {
    assert.equal(
      ...
      values.coordinate,
      "Install the exact public version, not a tag or range.",
    );
  ...
  assert.ok(
    (await readFile(join(installed, "LICENSE"), "utf8")).includes(
  ...
  for (const hook of ["preinstall", "install", "postinstall", "prepare"]) {
    assert.equal(
      ...
      undefined,
      "Consumers must not compile during installation.",
    );
  }
  for (const excluded of [
    ...
    assert.equal(
      await lstat(join(installed, excluded)).catch(() => undefined),
      undefined,
  ...
  for (const name of mapNames) {
    ...
    for (const source of map.sources) {
      ...
      assert.ok(
        ...
        "Declaration maps must resolve inside the shipped source.",
      );
    }
  }
  ...
  await writeFile(
    ...
    await readFile(
      new URL(
        "../test/package-consumers/javascript/public-api.js",
        import.meta.url,
      ),
    ),
  );
  ...
  const installedHelp = run(
    [npmCli, "exec", "--offline", "--", "steam-community-bbcode", "--help"],
    consumer,
  );
  ...
  const contracts = (await readdir(contractRoot)).filter((name) =>
    name.endsWith(".mts"),
  );
  ...
  for (const name of contracts) {
    await writeFile(
      join(consumer, name),
      await readFile(new URL(name, contractRoot)),
    );
  }
  await writeFile(
  ...
  if (values.audit) {
    ...
    assert.equal(
      audit.status,
      0,
      audit.error?.message ?? audit.stdout + audit.stderr,
    );
    await writeFile(
      join(artifacts, "production.cdx.json"),
      run(
        [
          npmCli,
          "sbom",
  ...
  if (values.coordinate) {
    await writeFile(
      join(artifacts, "signature-audit.json"),
      run(
        [
          npmCli,
          "audit",
  ...
  if (entry)
    await writeFile(
      join(artifacts, "pack-actual.json"),
