...
export const githubRenderingScenarios = [
  {
    ...
    markdown: [
      ...
      steamCommunityBbcodeToGfm(
        "[spoiler][b]Hidden[/b] *literal* <script>alert(1)</script>[/spoiler]",
      ).value,
      steamCommunityBbcodeToGfm(
        "[table][tr][th]H[/th][/tr][tr][td][strike]Old[/strike][/td][/tr][/table]",
      ).value,
    ].join("\n\n"),
    expectedHtml: [
  ...
  },
  {
    ...
    markdown: [
      ...
      steamCommunityBbcodeToGfm("[noparse]https://example.org/path[/noparse]")
        .value,
