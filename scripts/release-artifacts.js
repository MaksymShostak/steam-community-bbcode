...
export const releaseSignerPolicy = Object.freeze({
  ...
  certificateIdentityURI:
    "^https://github\\.com/MaksymShostak/steam-community-bbcode/\\.github/workflows/steam-community-bbcode-release\\.yml@refs/heads/main$",
});
...
export function assertPublicRegistryMetadata({
  ...
  if (distTags[tag] !== expectedVersion) {
    throw new Error(
      "The requested distribution tag does not identify the expected version.",
    );
  }
  ...
  if (
    ...
    !tarballUrl.startsWith(`${registry}${expectedName}/-/`)
  ) {
    throw new Error(
      "Public registry distribution metadata is incomplete or has the wrong origin.",
    );
  }
  return {
    coordinate: `${expectedName}@${expectedVersion}`,
    channel: tag,
    integrity,
    tarballUrl,
  };
 ...
 */
export function assertPublicRegistryFacts({
  expectedName,
  expectedVersion,
  tag,
  retainedSha256,
  ...
  const distribution = assertPublicRegistryMetadata({
    expectedName,
    expectedVersion,
    tag,
    metadata,
...
export function readCandidate(directory) {
  ...
  assert.equal(
    basename(candidate.tarball.filename),
    candidate.tarball.filename,
  );
  assert.equal(
    candidate.tarball.filename,
    `${candidate.package.name}-${candidate.package.version}.tgz`,
  );
  const archive = readFileSync(join(directory, candidate.tarball.filename));
  assert.equal(
    sha256Buffer(archive),
    candidate.tarball.sha256,
    "Retained candidate archive bytes changed.",
  );
...
function verifiedProvenanceBundle(candidate, audit) {
  ...
  assert.ok(
    ...
    "Native audit has no verified provenance entries.",
  );
  /** @type {unknown[]} */
  ...
  const matches = verified.filter((entry) => {
    assertRecord(entry);
    return (
      entry["name"] === candidate.package.name &&
      entry["version"] === candidate.package.version &&
  ...
  assert.equal(
    matches.length,
    1,
    "Expected exactly one verified target package provenance entry.",
  );
  ...
  assert.ok(
    ...
    "Native audit omitted verified provenance bundles.",
  );
  /** @type {unknown[]} */
  ...
  assert.equal(
    provenance.length,
    1,
    "Expected exactly one verified SLSA provenance statement.",
  );
  ...
  const bundle = /** @type {import('sigstore').Bundle} */ (
    attestation["bundle"]
  );
  assertRecord(bundle);
  return bundle;
...
function assertProvenanceClaims(candidate, bundle) {
  assert.ok(
    candidate.release,
    "Expected a hosted candidate for provenance verification.",
  );
  const identity = candidate.release;
  const repositoryUrl =
    "https://github.com/MaksymShostak/steam-community-bbcode";
  ...
  assert.deepEqual(statement["subject"], [
    {
      name: `pkg:npm/${candidate.package.name}@${candidate.package.version}`,
      digest: {
        sha512: Buffer.from(
          candidate.tarball.integrity.slice("sha512-".length),
          "base64",
        ).toString("hex"),
      },
    },
  ...
  assert.deepEqual(parameters["workflow"], {
  ...
  assert.deepEqual(definition["resolvedDependencies"], [
    {
      uri: `git+${repositoryUrl}@${identity.ref}`,
      digest: { gitCommit: candidate.source.commit },
    },
  ]);
  ...
  assert.equal(
    ...
    `${repositoryUrl}/actions/runs/${identity.runId}/attempts/${identity.runAttempt}`,
  );
}
