...
async function declarations() {
  ...
  for (const name of names) {
    if (name.endsWith(".d.ts") || name.endsWith(".d.ts.map")) {
      output.push([
        name,
        await readFile(new URL(name.replaceAll("\\", "/"), base), "utf8"),
      ]);
    }
  }
  assert.ok(
    output.length > 0,
    "Declaration generation must produce actual artifacts.",
  );
  return output;
}
...
assert.deepEqual(
  await declarations(),
  first,
  "Two clean builds must emit identical declarations and maps.",
);
console.log(
  `Verified ${first.length} declaration artifacts across two clean builds.`,
);
