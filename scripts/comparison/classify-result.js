...
export function classifyResult(observation) {
  ...
  if (!isDeepStrictEqual(observation.actualTree, observation.expectedTree)) {
    return observation.output.trim() === observation.source.trim()
      ? "FAIL_RAW_SOURCE_MARKUP"
      : "FAIL_WRONG_STRUCTURE";
  }
  if (observation.requiresDiagnosis)
    return observation.diagnostics.length
      ? "PASS_DIAGNOSED_LOSS"
      : "FAIL_SILENT_LOSS";
  return observation.output === observation.expectedOutput
    ? "PASS_EXACT"
    : "PASS_EQUIVALENT";
}
