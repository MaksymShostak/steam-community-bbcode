...
for (const name of ["common", "sources", "profiles", "constructs"]) {
  ...
  const schema = JSON.parse(
    await readFile(
      new URL(`../spec/schema/${name}.schema.json`, import.meta.url),
      "utf8",
    ),
  );
...
export function validateSpecification(documents) {
  ...
  const validateSources = ajv.getSchema(
    "https://steam-community-bbcode.invalid/schema/sources.schema.json",
  );
  /** @type {import('ajv').ValidateFunction<ProfileCollection> | undefined} */
  const validateProfiles = ajv.getSchema(
    "https://steam-community-bbcode.invalid/schema/profiles.schema.json",
  );
  /** @type {import('ajv').ValidateFunction<ConstructCollection> | undefined} */
  const validateConstructs = ajv.getSchema(
    "https://steam-community-bbcode.invalid/schema/constructs.schema.json",
  );
  if (!validateSources || !validateProfiles || !validateConstructs)
    throw new Error("Missing specification schema.");
  if (!validateSources(documents.sources))
    throw new Error(`sources: ${ajv.errorsText(validateSources.errors)}`);
  if (!validateProfiles(documents.profiles))
    throw new Error(`profiles: ${ajv.errorsText(validateProfiles.errors)}`);
  if (!validateConstructs(documents.constructs))
    throw new Error(`constructs: ${ajv.errorsText(validateConstructs.errors)}`);
  const { sources, profiles, constructs } = documents;
  if (
    sources.registryVersion !== profiles.registryVersion ||
    sources.registryVersion !== constructs.registryVersion
  ) {
  ...
  if (!profileIds.has(profiles.defaultProfile))
    throw new Error(`Unknown default profile: ${profiles.defaultProfile}`);
  for (const construct of constructs.constructs) {
    for (const provenance of construct.provenance) {
      if (!sourceIds.has(provenance.sourceId))
        throw new Error(`Unknown source: ${provenance.sourceId}`);
    }
    for (const profileId of Object.keys(construct.profiles)) {
      if (!profileIds.has(profileId))
        throw new Error(`Unknown construct profile: ${profileId}`);
    }
  }
...
function uniqueIdentities(entries, kind) {
  ...
  for (const entry of entries) {
    if (identities.has(entry.id))
      throw new Error(`Duplicate ${kind} identity: ${entry.id}`);
...
export async function readSpecificationDocuments() {
  const [sources, profiles, constructs] = await Promise.all(
    ["sources", "profiles", "constructs"].map(async (name) => {
      /** @type {unknown} */
      const document = JSON.parse(
        await readFile(
          new URL(`../spec/${name}.json`, import.meta.url),
          "utf8",
        ),
      );
      return document;
...
if (
  ...
  console.log(
    "Specification schemas and references validated; conformance is not assessed.",
  );
}
