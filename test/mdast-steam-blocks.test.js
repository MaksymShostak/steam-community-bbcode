...
test("attribution and historical color survive in dedicated MDAST extensions", () => {
  const result = steamCommunityBbcodeToMdast(
    '[quote="Ada;42"][color=#123456]Text[/color][/quote][pullquote]Aside[/pullquote]',
  );
...
test("block spoilers retain nested lists and quotes before target lowering", () => {
  const source =
    "[spoiler][list][*]Hidden[/list][quote]Quoted[/quote][/spoiler]";
  ...
  assert.equal(
    tree.children[0]?.type === "html" ? tree.children[0].value : "",
    "<details>\n<summary>Spoiler</summary>",
  );
  assert.equal(tree.children[1]?.type, "list");
  assert.equal(tree.children[2]?.type, "blockquote");
  assert.ok(
    gfm.diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_SPOILER_LOWERED_TO_DETAILS",
    ),
  );
});
...
test("target lowering retains quoted text and attribution while reporting presentation loss", () => {
  const result = steamCommunityBbcodeToGfm(
    "[quote=Ada;42][color=red]Text[/color][/quote][pullquote]Aside[/pullquote]",
  );
  ...
  assert.ok(
    result.diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_QUOTE_METADATA_LOWERED",
    ),
  );
  assert.ok(
    result.diagnostics.some(
      (diagnostic) =>
        diagnostic.code === "STEAM_COLOR_LOWERED_TO_TEXT" &&
  ...
  assert.ok(
    result.diagnostics.some(
      (diagnostic) =>
        diagnostic.code === "STEAM_PULLQUOTE_LOWERED_TO_BLOCKQUOTE",
    ),
  );
});
