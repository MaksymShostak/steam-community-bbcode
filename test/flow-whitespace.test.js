...
test("flow-whitespace between explicit blocks does not become duplicate paragraph padding", () => {
  ...
  assert.deepEqual(
    fromMarkdown(result.value).children.map((child) => child.type),
    ["heading", "paragraph", "thematicBreak"],
  );
...
test("flow-whitespace retains nested list ownership and paragraphs within an item", () => {
  const source =
    "[list]\n[*] Parent\n[olist]\n[*] Child\n[/olist]\n[*] First paragraph.\n\nSecond paragraph.\n[/list]";
  const list = fromMarkdown(steamCommunityBbcodeToGfm(source).value)
    .children[0];
  assert.ok(list?.type === "list");
  assert.equal(list.children.length, 2);
  ...
  assert.deepEqual(
    list.children[1]?.children.map((child) => child.type),
    ["paragraph", "paragraph"],
  );
});
...
test("flow-whitespace keeps opaque padding, code content and malformed fallback", () => {
  ...
  const paragraph = fromMarkdown(steamCommunityBbcodeToGfm(source).value)
    .children[1];
  assert.ok(paragraph?.type === "paragraph");
  const text = paragraph.children[0];
  assert.ok(text?.type === "text");
...
test("flow-whitespace keeps literal fallback and source positions around a link", () => {
  ...
  assert.equal(
    ...
    "[url=https://example.org] Label [/url]",
  );
...
for (const separator of ["\u00a0", "\u2003"]) {
  for (const boundary of ["table", "row"]) {
    test(`table layout preserves ${JSON.stringify(separator)} at its ${boundary} boundary`, () => {
      ...
      const positive = steamCommunityBbcodeToMdast(
        `[table] \t\r\n${header}\n${row}[/table]`,
      );
      assert.equal(
        positive.value.children[0]?.type,
        "table",
        "the fixture must otherwise be a representable table",
      );
      const source =
        boundary === "table"
      ...
      assert.ok(
        result.diagnostics.some(
          (diagnostic) => diagnostic.code === "STEAM_TABLE_STRUCTURE_PRESERVED",
        ),
      );
      assert.ok(
        result.coverage.constructs.every(
          (outcome) => outcome.fidelity === "unsupported",
        ),
      );
    });
  }
