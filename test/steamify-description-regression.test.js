...
const bytes = await readFile(
  new URL("./fixtures/steamify/original-description.bbcode", import.meta.url),
);
assert.equal(
  createHash("sha256").update(bytes).digest("hex"),
  "d2d80f2e374a2eb7360d480cd139c458c1404f6ae4592e4131c009e452eb8a3c",
);
const source = bytes.toString("utf8");
const converted = steamCommunityBbcodeToGfm(source);
const tree = fromMarkdown(converted.value, {
  extensions: [gfm()],
...
test("Steamify actual: deterministic", () => {
  for (let i = 0; i < 10; i += 1)
    assert.equal(steamCommunityBbcodeToGfm(source).value, converted.value);
});
test("Steamify actual: CRLF and LF equivalent", () => {
  assert.equal(
    steamCommunityBbcodeToGfm(source.replace(/\n/gu, "\r\n")).value,
    converted.value,
  );
});
for (const [tag, kind, expected] of /** @type {const} */ ([
  ["h1", "heading", 6],
  ["b", "strong", 29],
  ["i", "emphasis", 5],
  ["list", "list", 4],
...
test("Steamify actual: nested italic link", () => {
  const link = all.find(
    (node) => node.type === "link" && node.url === "FAST_TRACK_LINK",
  );
  assert.ok(link?.type === "link");
  const emphasis = link.children[0];
  assert.ok(emphasis?.type === "emphasis");
  assert.equal(
    emphasis.children[0]?.type === "text" ? emphasis.children[0].value : "",
    "Fast Track",
  );
});
for (const literal of [
  ...
  test(`Steamify actual: literal ${literal}`, () => {
    ...
    assert.ok(
      all.some((node) => node.type === "text" && node.value.includes(literal)),
    );
  });
}
