...
test("literal URL escaping survives syntax parsing and reports additional GitHub autolinking", () => {
  ...
  const parsed = fromMarkdown(result.value, {
    extensions: [gfm()],
    mdastExtensions: syntax,
  });
  const paragraph = parsed.children[0];
  assert.ok(paragraph?.type === "paragraph");
  assert.deepEqual(
    paragraph.children.map((n) => n.type),
    ["text"],
  );
  const rendered = fromMarkdown(result.value, {
    extensions: [gfm()],
  ...
  assert.ok(
    result.diagnostics.some(
      (d) => d.code === "GFM_RENDERER_AUTOLINK_POSSIBLE" && d.scope === "input",
    ),
  );
  const explicit = steamCommunityBbcodeToGfm(
    "[url=https://example.org/path]Link[/url]",
  );
  assert.ok(
    !explicit.diagnostics.some(
      (d) => d.code === "GFM_RENDERER_AUTOLINK_POSSIBLE",
    ),
  );
});
...
const rendererCases = [
  ...
  [
    "[table][tr][th]H[/th][/tr][tr][td][noparse]www.example.org[/noparse][/td][/tr][/table]",
    true,
  ],
...
test("the native renderer transform diagnoses a copied tree without changing its literal content", () => {
  ...
  const intended = {
    ...
    children: [
      {
        type: "paragraph",
        children: [{ type: "text", value: "www.example.org" }],
      },
    ],
  };
