...
test("all 33 historical checks remain identifiable independently of new conversion results", async () => {
  ...
  const actual = history.checks.filter((check) =>
    check.name.startsWith("actual: "),
  );
  ...
  for (const probe of probes) {
    ...
    assert.ok(
      typeof probe.detail !== "string" && probe.detail.input.length > 0,
    );
  }
});
...
test("independently authored description exercises the same structural preconditions", async () => {
  const source = await readFile(
    new URL("./fixtures/steamify/description.bbcode", import.meta.url),
    "utf8",
  );
