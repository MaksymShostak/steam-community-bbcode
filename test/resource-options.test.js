...
for (const convert of [parseSteamCommunityBbcode, gfmToSteamCommunityBbcode]) {
  test(`${convert.name} rejects invalid resource options with actionable caller errors`, () => {
    for (const resourceLimits of [null, [], 0, 1, true, false, "", "limits"]) {
      assert.throws(
        () => Reflect.apply(convert, undefined, ["", { resourceLimits }]),
        { name: "TypeError", message: /Resource limits must be an object/ },
      );
    }
    assert.throws(
      () =>
        Reflect.apply(convert, undefined, [
          "",
    ...
    assert.throws(
      () => convert("", { resourceLimits: { maxNestingDepth: 257 } }),
...
test("public conversion boundaries identify wrong source and option arguments", () => {
  for (const convert of [
    ...
    for (const source of [undefined, null, 1, true, []]) {
      assert.throws(() => Reflect.apply(convert, undefined, [source]), {
        name: "TypeError",
        message: /source.*string/i,
      });
    }
    for (const options of [null, [], 1, true, ""]) {
      assert.throws(() => Reflect.apply(convert, undefined, ["", options]), {
        name: "TypeError",
        message: /options.*object/i,
      });
    }
    assert.throws(
      () => Reflect.apply(convert, undefined, ["", { misspelled: true }]),
      { name: "TypeError", message: /Unknown.*option.*misspelled/i },
    );
  }
  ...
  for (const options of [null, [], 1, true, ""]) {
    assert.throws(
      () =>
        Reflect.apply(steamCommunityBbcodeToMdast, undefined, [
          prepared,
          options,
        ]),
  ...
  assert.throws(
    () =>
      Reflect.apply(steamCommunityBbcodeToMdast, undefined, [{ ...prepared }]),
    { name: "TypeError", message: /issued by parseSteamCommunityBbcode/ },
  );
  assert.throws(
    () => steamCommunityBbcodeToMdast(prepared, { profile: "review" }),
    { name: "RangeError", message: /profile cannot be changed/ },
  );
  assert.throws(
    () =>
      steamCommunityBbcodeToMdast(prepared, {
        resourceLimits: { maxInputBytes: 1 },
      }),
  ...
  assert.deepEqual(
    steamCommunityBbcodeToMdast(prepared, { profile: "workshop-item" })
      .diagnostics,
    [],
  );
});
