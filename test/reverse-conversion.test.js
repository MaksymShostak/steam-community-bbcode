...
function targetTree(steam) {
  ...
  return semanticProjection(
    fromMarkdown(steamCommunityBbcodeToGfm(steam).value, {
      extensions: [gfm()],
...
test("reverse preserves heading and inline semantics while protecting literal BBCode", () => {
  const result = gfmToSteamCommunityBbcode(
    "# A **bold** and *italic* ~~gone~~\n\nPlain [b]literal[/b].",
  );
  ...
  const target = fromMarkdown(steamCommunityBbcodeToGfm(result.value).value, {
    extensions: [gfm()],
    mdastExtensions: gfmFromMarkdown(),
  });
  assert.deepEqual(semanticProjection(target), {
    type: "root",
    children: [
      {
        type: "heading",
      ...
      {
        type: "paragraph",
        children: [{ type: "text", value: "Plain [b]literal[/b]." }],
      },
    ],
  });
  ...
  assert.ok(
    result.coverage.sourceNodes.some(
      (node) => node.nodeType === "strong" && node.fidelity === "equivalent",
    ),
  );
});
...
for (const [nodeType, source] of [
  ["heading", "#### Deeper"],
  ["html", '<script>alert("[b]not a tag[/b]")</script>'],
  ["inlineCode", "`x [b] y`"],
  ["list", "- [x] Complete"],
  ["list", "7. Starts at seven"],
  ["code", "```js\nconst x = 1;\n```"],
  ["code", "```\n[/CoDe ] [b]literal[/b]\n```"],
  ["image", "![Visible alt](https://example.org/image.png)"],
  ["link", '[Label](https://example.org "Title")'],
  ["link", "[bad](javascript:alert%281%29)"],
  ["link", "[bad](https://example.org/%22[img])"],
  ["link", "[Relative](./guide.md)"],
  ["image", "![](../image.png)"],
  ...
  test(`reverse preserves unsupported ${nodeType}: ${source}`, () => {
    ...
    assert.ok(
      result.unsupportedSourceNodes.some(
        (d) =>
          d.nodeType === nodeType &&
          d.fidelity === "unsupported" &&
          d.sourceSpan,
      ),
    );
    assert.deepEqual(parseSteamCommunityBbcode(result.value).diagnostics, []);
    assert.deepEqual(targetTree(result.value), {
      type: "root",
      children: [
...
test("reverse literal encoding cannot be terminated by any case of noparse or injected active tags", () => {
  ...
  assert.deepEqual(syntax.diagnostics, []);
  assert.ok(
    syntax.children.every(
      (n) => n.type === "steamOpaqueTag" && n.tagName === "noparse",
    ),
  );
  assert.deepEqual(targetTree(result.value), {
    type: "root",
...
for (const [source, limits, code] of [
  ...
  test(`reverse enforces ${code} with an input-scoped failure`, () => {
    assert.ok(
      typeof source === "string" && limits && typeof limits === "object",
    );
    const result = gfmToSteamCommunityBbcode(source, {
    ...
    assert.ok(
      result.diagnostics.some(
        (d) => d.scope === "input" && d.severity === "error" && d.code === code,
      ),
    );
...
test("reverse accepts exact UTF-8 and tree limits and rejects invalid JavaScript options", () => {
  const exact = gfmToSteamCommunityBbcode("😀", {
    resourceLimits: {
      maxInputBytes: 4,
      maxNodeCount: 3,
      maxNestingDepth: 2,
  ...
  for (const options of [
    ...
    { unknown: true },
    { resourceLimits: null },
    { resourceLimits: { maxInputBytes: 0 } },
    { resourceLimits: { maxOutputBytes: Infinity } },
    { resourceLimits: { maxNestingDepth: 257 } },
    { resourceLimits: { unknown: 1 } },
  ]) {
    assert.throws(() =>
      Reflect.apply(gfmToSteamCommunityBbcode, undefined, ["x", options]),
    );
  }
});
...
test("reverse qualifies the supported renderer depth and reports the next level", () => {
  ...
  const permitted = gfmToSteamCommunityBbcode(source, {
    resourceLimits: { maxNestingDepth: 256 },
  });
  assert.deepEqual(permitted.diagnostics, []);
  assert.deepEqual(
    parseSteamCommunityBbcode(permitted.value, {
      resourceLimits: { maxNestingDepth: 256 },
    }).diagnostics,
    [],
  );
  const exceeded = gfmToSteamCommunityBbcode("> " + source, {
    resourceLimits: { maxNestingDepth: 256 },
  });
    ...
    children: [
      { type: "code", value: "[b]literal[/b]" },
      {
        ...
        children: [
          {
            type: "link",
            url: "https://example.org/a?q=1&b=2",
            children: [
          ...
          { type: "text", value: " " },
          { type: "image", url: "https://example.org/a.png", alt: "" },
        ],
      },
      {
        type: "table",
...
test("reverse retains mixed inline siblings and distinct paragraphs inside quotes and list items", () => {
  const source =
    "> First **bold *inner* tail**.\n>\n> Second.\n\n- First paragraph.\n\n  Second paragraph.\n\n| H |\n| - |\n| **B** tail |";
  const result = gfmToSteamCommunityBbcode(source);
  assert.deepEqual(result.unsupportedSourceNodes, []);
  const expected = semanticProjection(
    fromMarkdown(source, {
      extensions: [gfm()],
      mdastExtensions: gfmFromMarkdown(),
    }),
...
test("one unsupported child or column keeps its enclosing reverse structure literal", () => {
  for (const source of [
    ...
    assert.ok(
      result.coverage.sourceNodes
        .filter((node) => node.nodeType !== "root")
        .every((node) => node.fidelity === "unsupported"),
    );
    assert.deepEqual(targetTree(result.value), {
      type: "root",
      children: [
