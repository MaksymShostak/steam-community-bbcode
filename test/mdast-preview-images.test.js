...
test("a screenshot keeps its explicit URL and alt text while accounting for Steam presentation", () => {
  const source =
    "[screenshot=420;https://example.org/screen.png]A *literal* view[/screenshot]";
  ...
  assert.deepEqual(preview.image, {
    kind: "url",
    url: "https://example.org/screen.png",
    steamImageId: "420",
  });
  ...
  assert.ok(
    result.diagnostics.some(
      (d) =>
        d.code === "STEAM_PREVIEW_IMAGE_PRESENTATION_OMITTED" &&
...
test("a guide image remains an unresolved reference rather than a fabricated URL", () => {
  ...
  assert.ok(target?.type === "paragraph");
  assert.deepEqual(
    target.children.map((n) => (n.type === "text" ? n.value : n.type)),
    [source],
  );
  assert.ok(
    result.diagnostics.some(
      (d) =>
        d.code === "STEAM_GUIDE_IMAGE_UNRESOLVED" &&
...
for (const [attributes, size, alignment] of [
  ...
  test(`guide image layout variants retain their identity without inventing a URL: ${attributes}`, () => {
    ...
    assert.ok(
      result.diagnostics.some((d) => d.code === "STEAM_GUIDE_IMAGE_UNRESOLVED"),
    );
  });
}
