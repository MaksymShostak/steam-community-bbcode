...
test("opaque source and unknown attributes survive the public syntax boundary", () => {
  const source =
    '[futuretag key="a]b"][noparse][b]literal[/b][/noparse][/futuretag]';
  const result = parseSteamCommunityBbcode(source, {
...
test("public parsing rejects invalid profiles and options instead of changing their meaning", () => {
  for (const options of [
    { profile: null },
    { profile: "workshop" },
    { profil: "workshop-item" },
    null,
    [],
  ]) {
    assert.throws(
      () => Reflect.apply(parseSteamCommunityBbcode, undefined, ["", options]),
...
test("public source, syntax, spans and diagnostics cannot be mutated", () => {
  ...
  assert.equal(
    Reflect.set(parsed.diagnostics[0] ?? {}, "code", "changed"),
    false,
  );
});
...
test("unpaired unknown bracket labels do not absorb known formatting or its closer", () => {
  ...
  assert.equal(
    parsed.children.map((child) => child.rawSource).join(""),
    source,
  );
});
...
test("opaque and horizontal-rule whitespace retains normalized tag names and complete source positions", () => {
  ...
  assert.deepEqual(
    result.children.map((node) => node.rawSource),
    parts,
  );
  assert.deepEqual(
    result.children.map((node) => node.type),
    [
      "steamOpaqueTag",
      "steamTag",
      "steamOpaqueTag",
      "steamUnmatchedClosingTag",
    ],
  );
  assert.deepEqual(
    result.children.map((node) => ("tagName" in node ? node.tagName : "")),
    ["code", "hr", "noparse", "b"],
  );
  ...
  assert.deepEqual(
    result.children.map((node) => node.sourceSpan),
    [
      {
        start: { line: 1, column: 1, offset: 0 },
        end: { line: 4, column: 11, offset: 23 },
      },
      {
        start: { line: 4, column: 11, offset: 23 },
        end: { line: 4, column: 23, offset: 35 },
      },
      {
        start: { line: 4, column: 23, offset: 35 },
        end: { line: 4, column: 47, offset: 59 },
      },
      {
        start: { line: 4, column: 47, offset: 59 },
        end: { line: 4, column: 53, offset: 65 },
      },
    ],
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    result.diagnostics[0]?.sourceSpan,
    result.children[3]?.sourceSpan,
  );
});
...
test("adjacent opaque and boundary constructs release depth and respect the exact node quota", () => {
  ...
  const result = parseSteamCommunityBbcode(source, {
    resourceLimits: { maxNestingDepth: 1, maxNodeCount: 4 },
  });
  assert.deepEqual(result.diagnostics, []);
  assert.equal(result.children.length, 4);
  assert.equal(result.children.map((node) => node.rawSource).join(""), source);
  assert.equal(
    parseSteamCommunityBbcode(source, { resourceLimits: { maxNodeCount: 3 } })
      .diagnostics[0]?.code,
...
test("unfinished quoted attributes retain source and explain each syntax problem", () => {
  for (const quote of ['"', "'"]) {
    ...
    assert.equal(
      parsed.children.map((node) => node.rawSource).join(""),
      source,
    );
    ...
    for (const diagnostic of parsed.diagnostics) {
      assert.notEqual(
        ...
        "",
        `Missing explanation: ${diagnostic.code}`,
      );
      assert.ok(diagnostic.sourceSpan);
    }
  }
...
test("an unfinished header counts only its attribute bytes and rejects overflow without a partial tree", () => {
  ...
  const accepted = parseSteamCommunityBbcode(source, {
    resourceLimits: { maxAttributeBytes: 4 },
  });
  assert.equal(
    accepted.children.map((node) => node.rawSource).join(""),
    source,
  );
  assert.ok(
    accepted.diagnostics.some(
      (d) => d.code === "STEAM_UNCLOSED_ATTRIBUTE_QUOTE",
    ),
  );
  assert.ok(
    accepted.diagnostics.every(
      (d) => d.code !== "STEAM_MAX_ATTRIBUTE_BYTES_EXCEEDED",
    ),
  );

  const rejected = parseSteamCommunityBbcode(source, {
    resourceLimits: { maxAttributeBytes: 3 },
  });
  assert.equal(rejected.source, source);
  assert.deepEqual(rejected.children, []);
  assert.deepEqual(
    rejected.diagnostics.map((d) => d.code),
