...
test("source Markdown punctuation remains literal MDAST text", () => {
  ...
  assert.ok(
    result.diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_UNKNOWN_CONSTRUCT",
    ),
  );
});
...
test("headings and nested emphasis use the native MDAST node model", () => {
  ...
  assert.deepEqual(
    heading.children.map((child) => (child.type === "text" ? child.value : "")),
    ["Title"],
  );
...
test("prepared source must be issued by the parser and retain its parsing contract", () => {
  ...
  assert.throws(
    () => Reflect.apply(steamCommunityBbcodeToMdast, undefined, [forged]),
    TypeError,
  );
  const issued = parseSteamCommunityBbcode("X");
  assert.throws(
    () => steamCommunityBbcodeToMdast(issued, { profile: "review" }),
    RangeError,
  );
  assert.throws(
    () =>
      steamCommunityBbcodeToMdast(issued, {
