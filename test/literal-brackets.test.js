...
const literalLabels = [
  ["[sd] QooLiO", "[sd] QooLiO"],
  [
    "[i]Delivery Temperature Limit [Fixed][/i]",
    "Delivery Temperature Limit [Fixed]",
  ],
  [
    "[url=https://example.org][i]Delivery Temperature Limit [Fixed][/i][/url]",
    "Delivery Temperature Limit [Fixed]",
  ],
...
for (const [source, expectedText] of literalLabels) {
  test(`literal-bracket labels have one accurate preservation diagnostic: ${source}`, () => {
    ...
    assert.match(
      source.slice(
        diagnostic.sourceSpan.start.offset,
        diagnostic.sourceSpan.end.offset,
      ),
      /^\[(?:sd|Fixed|FutureLabel)\]$/,
    );
    ...
    if (source.startsWith("[i]"))
      assert.equal(paragraph.children[0]?.type, "emphasis");
    if (source.startsWith("[FutureLabel]"))
...
test("literal-bracket labels inside noparse remain opaque without warnings", () => {
  const result = steamCommunityBbcodeToGfm(
    "[noparse][sd] [Fixed] [b]literal[/b][/noparse]",
  );
