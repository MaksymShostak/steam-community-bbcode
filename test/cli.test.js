...
test("CLI fidelity thresholds reject the selected loss or worse while retaining the result", () => {
  ...
  assert.equal(
    cli(["to-gfm", "--fail-on=lossy"], "[spoiler]S[/spoiler]").status,
    0,
  );
  assert.equal(
    cli(["to-gfm", "--fail-on=approximate"], "[spoiler]S[/spoiler]").status,
    1,
  );
  const unsupported = cli(
    ["to-steam", "--fail-on=lossy", "--format=json"],
    "#### H",
  );
...
test("CLI emits the shipped conformance report and selected forward profile", () => {
  ...
  const conversion = cli(
    ["to-gfm", "--profile=discussion", "--format=json"],
    "[b]B[/b]",
  );
...
for (const args of [
  ...
  ["to-gfm", "--format=xml"],
  ["to-gfm", "--profile=typo"],
  ["to-steam", "--profile=workshop-item"],
  ["to-gfm", "--fail-on=typo"],
  ["to-gfm", "--max-input-bytes=0"],
  ["to-gfm", "--max-input-bytes=1.5"],
  ["coverage", "unexpected.bbcode"],
  ["coverage", "--fail-on=lossy"],
  ["coverage", "--format=json", "--fail-on=lossy"],
...
test("CLI reports invalid byte-limit arguments before attempting to read a missing input", () => {
  for (const limit of ["0", "-1", "1.5", "Infinity", "9007199254740992"]) {
    const result = cli([
      "to-gfm",
      `--max-input-bytes=${limit}`,
      "does-not-exist.bbcode",
    ]);
    assert.equal(result.status, 2);
    assert.equal(result.stdout, "");
...
test("an explicit input limit above the default reaches both public converters", () => {
  ...
  const forward = cli(
    ["to-gfm", "--max-input-bytes=1048600"],
    `[code]${payload}[/code]`,
  );
  assert.equal(forward.status, 0);
  assert.equal(forward.stdout, "```\n" + payload + "\n```\n");
  const reverse = cli(
    ["to-steam", "--max-input-bytes=1048600"],
    "```\n" + payload + "\n```",
  );
...
test("CLI reports missing files without a stack trace or partial output", () => {
  const result = cli([
    "to-gfm",
    fileURLToPath(
      new URL("./fixtures/cli/does-not-exist.bbcode", import.meta.url),
    ),
  ]);
  assert.equal(result.status, 2);
  assert.equal(result.stdout, "");
...
test(
  "CLI reports a closed output pipe as an I/O failure without an unhandled stack",
  { timeout: 10000 },
  async () => {
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../src/cli.js", import.meta.url)), "to-gfm"],
      { windowsHide: true },
    );
    let stderr = "";
...
test(
  "CLI retains an I/O failure exit code when the diagnostic pipe is also closed",
  { timeout: 10000 },
  async () => {
    const child = spawn(
      process.execPath,
...
test("CLI keeps repaired formatting exact and rejects diagnosed nonbreaking table preservation", () => {
  const formatting = cli(
    ["to-gfm", "--fail-on=approximate"],
    "[b]a[b]b[/b][/b]\n\n[strike] D [/strike]",
  );
