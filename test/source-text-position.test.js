...
test("successive text slices retain UTF-16 positions across CRLF, LF and CR", () => {
  ...
  const value = "😀\r\nA\nB\rC:ok:";
  const consume = sourceTextPositionCursor({
    type: "steamText",
    value,
    rawSource: value,
  ...
  assert.deepEqual(
    last,
    { start: crEnd, end },
    "later consumption must not mutate previously returned positions",
  );
  assert.deepEqual(
    start,
    { line: 4, column: 3, offset: 20 },
    "the source span is immutable input",
  );
});
