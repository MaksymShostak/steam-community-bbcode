...
test("SDLC aggregate rejects unsuccessful or missing matrix results through its actual shell", () => {
  ...
  assert.equal(
    controls.getIn(["jobs", "required", "steps", 0, "env", "CONTROL_RESULT"]),
    "${{ needs.controls.result }}",
  );
  ...
  for (const state of ["success", "failure", "cancelled", "skipped", ""]) {
    ...
    const result = spawnSync(
      ...
      ["--noprofile", "--norc", "-eo", "pipefail", "-c", command],
      {
        encoding: "utf8",
        windowsHide: true,
        env: { ...process.env, CONTROL_RESULT: state },
      },
    );
...
test("PR linkage executes only trusted base policy with read-only access", () => {
  ...
  assert.equal(
    linkage.getIn(["jobs", "validate", "steps", 0, "with", "ref"]),
    "${{ github.sha }}",
  );
  assert.equal(
    linkage.getIn([
  ...
  assert.ok(
    permissions.items.every(
      (pair) => isScalar(pair.value) && pair.value.value === "read",
    ),
  );
  ...
  for (const step of steps.items) {
    ...
    if (typeof command === "string")
      assert.doesNotMatch(
        command,
        /head\.sha|pull_request\.head|checkout|git fetch/u,
      );
  }
});
