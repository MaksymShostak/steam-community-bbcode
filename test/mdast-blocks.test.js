  ...
  test(`native ${tag} hierarchy retains a nested child and following sibling`, () => {
    ...
    assert.equal(
      result.coverage.constructs.filter(
        (outcome) => outcome.constructId === "steam.bbcode.list-item",
      ).length,
      3,
    );
  });
...
test("code remains an exact opaque value beside explicit blocks and strikethrough", () => {
  const result = steamCommunityBbcodeToMdast(
    "[p][strike]Old[/strike][/p][hr][/hr][quote]Outer[quote]Inner[/quote][/quote][code]\n```\ntext\n[/code]",
  );
  assert.deepEqual(result.diagnostics, []);
...
test("orphan list markers and non-item list prose are retained with a diagnostic", () => {
  ...
  assert.ok(
    result.diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_LIST_CONTENT_PRESERVED",
    ),
  );
