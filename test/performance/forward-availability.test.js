...
const workloads = [
  [
    "opaque 16000",
    "[noparse]" + "<[x]>".repeat(16000) + "[/noparse]",
    "<\\[x]>".repeat(16000) + "\n",
  ],
  [
    "opaque 32000",
    "[noparse]" + "<[x]>".repeat(32000) + "[/noparse]",
    "<\\[x]>".repeat(32000) + "\n",
  ],
  [
    "adjacent bold 40000",
...
for (const [name, input, expected] of workloads) {
  test(`forward availability: ${name}`, (context) => {
    ...
    assert.equal(
      child.error,
      undefined,
      "conversion must finish within the fixed subprocess budget",
    );
    ...
    context.diagnostic(
      `inputBytes=${Buffer.byteLength(input)} elapsedMs=${result.elapsedMs}`,
    );
  });
}
