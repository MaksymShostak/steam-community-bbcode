...
test("comparison distinguishes exact bytes from equivalent target structure", () => {
  ...
  assert.equal(
    classifyResult({
      ...observation,
      output: "__B__",
      actualTree: structuredClone(expectedTree),
    }),
    "PASS_EQUIVALENT",
  );
});
...
test("diagnostics account for known target loss but cannot excuse wrong structure", () => {
  assert.equal(
    classifyResult({ ...observation, requiresDiagnosis: true }),
    "FAIL_SILENT_LOSS",
  );
  assert.equal(
    classifyResult({
...
test("unchanged source is raw markup only when target semantics are wrong", () => {
  assert.equal(
    classifyResult({
      ...
      actualTree: { type: "text", value: observation.source },
    }),
    "FAIL_RAW_SOURCE_MARKUP",
  );
  assert.equal(
    classifyResult({ ...observation, source: "**B**" }),
    "PASS_EXACT",
  );
});
...
test("unclaimed capability and execution errors remain separate from conversion failures", () => {
  assert.equal(
    classifyResult({ ...observation, error: "TypeError: legacy dependency" }),
    "FAIL_EXCEPTION",
  );
  assert.equal(
    classifyResult({
