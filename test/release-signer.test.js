...
test("native Sigstore verifies a real bundle but rejects its different authenticated workflow", async () => {
  ...
  const bundle = JSON.parse(
    readFileSync(
      new URL(
        "./fixtures/release/sigstore-5.0.0-provenance.json",
        import.meta.url,
      ),
      "utf8",
    ),
  ...
  try {
    ...
    const original = await createVerifier({
      ...releaseSignerPolicy,
      certificateIdentityURI: upstreamIdentity,
      ...cacheOptions,
    });
    assert.ok(
      original.verify(bundle).identity,
      "Fixture must have a valid native signature and authenticated identity.",
    );
    const release = await createVerifier({
      ...releaseSignerPolicy,
      ...cacheOptions,
    });
    assert.throws(() => release.verify(bundle), {
      code: "UNTRUSTED_SIGNER_ERROR",
    });
    const wrongIssuer = await createVerifier({
      ...releaseSignerPolicy,
      certificateIdentityURI: upstreamIdentity,
      certificateIssuer: "https://accounts.google.com",
      ...cacheOptions,
    });
    assert.throws(() => wrongIssuer.verify(bundle), {
      code: "UNTRUSTED_SIGNER_ERROR",
    });
  } finally {
...
test("native signer identity policy matches exactly rather than accepting regex lookalikes", () => {
  ...
  for (const value of [
    ...
    identity.replace(".yml", "Xyml"),
    identity.replace("/main", "/other"),
  ]) {
