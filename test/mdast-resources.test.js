    ...
    for (const kind of /** @type {const} */ (["link", "image"])) {
      assert.equal(
        isAllowedResourceUrl(
          `https://example.org/a${String.fromCharCode(code)}b`,
          kind,
        ),
        false,
      );
...
test("HTTP and relative resources retain spelling; mailto is an active link only", () => {
  for (const destination of [
    ...
    const result = steamCommunityBbcodeToMdast(
      `[url=${destination}]Label[/url][img]${destination}[/img]`,
    );
  ...
  for (const source of [
    ...
    assert.ok(paragraph?.type === "paragraph");
    assert.equal(
      paragraph.children[0]?.type,
      source.startsWith("!") ? "image" : "link",
    );
  }
  assert.equal(
    gfmToSteamCommunityBbcode("![](mailto:person@example.org)")
      .unsupportedSourceNodes[0]?.nodeType,
    "image",
  );
});
...
test("attributed links retain nested formatting and images; bare URLs retain their label", () => {
  const result = steamCommunityBbcodeToMdast(
    '[url=FAST_TRACK_LINK][i]Fast Track[/i][/url] [url="https://example.org/a?x=1&y=2"][img]https://example.org/badge.png[/img][/url] [url]https://example.org[/url]',
  );
  assert.deepEqual(result.diagnostics, []);
  const paragraph = result.value.children[0];
  assert.ok(paragraph?.type === "paragraph");
  ...
  assert.equal(
    links[2]?.children[0]?.type === "text" ? links[2].children[0].value : "",
    "https://example.org",
  );
});
  ...
  test(`unsafe resource remains literal: ${JSON.stringify(destination)}`, () => {
    ...
    assert.equal(
      result.diagnostics.filter(
        (diagnostic) => diagnostic.code === "STEAM_UNSAFE_URL_PRESERVED",
      ).length,
      2,
    );
  });
...
test("a nested link is preserved within its outer label with explicit accounting", () => {
  const result = steamCommunityBbcodeToMdast(
    "[url=https://outer.example]A[url=https://inner.example]B[/url][/url]",
  );
  const paragraph = result.value.children[0];
  assert.ok(paragraph?.type === "paragraph");
  ...
  assert.ok(link?.type === "link");
  assert.equal(
    link.children.some((child) => child.type === "link"),
    false,
  );
  assert.ok(
    result.diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_NESTED_LINK_PRESERVED",
    ),
  );
});
...
test("single-quoted destinations accept spacing around the primary attribute", () => {
  const result = steamCommunityBbcodeToMdast(
    "[url = 'https://example.org/a%20b']Label[/url]",
  );
  const paragraph = result.value.children[0];
  assert.ok(paragraph?.type === "paragraph");
  const link = paragraph.children[0];
...
test("a resource body cannot absorb active markup into its destination", () => {
  for (const source of [
    "[url]https://example.org/[b]path[/b][/url]",
    "[img]https://example.org/[b]image[/b][/img]",
  ]) {
    const result = steamCommunityBbcodeToMdast(source);
    const paragraph = result.value.children[0];
    assert.ok(paragraph?.type === "paragraph");
    ...
    assert.ok(
      result.diagnostics.some(
        (d) => d.code === "STEAM_CONSTRUCT_PRESERVED" && d.message.length > 0,
      ),
    );
  }
});
