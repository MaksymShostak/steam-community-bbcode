...
test("the native grammar preserves nested tags and Steam item boundaries", () => {
  const source =
    "[olist][*]Outer[b]bold[/b][olist][*]Inner[/olist][*]Next[/olist]";
  ...
  assert.equal(
    result.tokens.filter((token) => token.image === "[*]").length,
    3,
  );
  assert.equal(
    result.tokens.filter((token) => token.image === "[olist").length,
    2,
  );
  ...
  assert.equal(
    rootEntries?.length,
    1,
    "The outer list owns all nested content.",
  );
  ...
  assert.equal(
    nestedTag.children["element"]?.length,
    2,
    "The nested list contains one boundary and its text.",
  );
});
...
test("opaque code and noparse preserve inner markup and backticks literally", () => {
  ...
  assert.equal(
    result.tokens
      .filter((token) => token.tokenType.name === "CodeText")
  ...
  assert.equal(
    result.tokens
      .filter((token) => token.tokenType.name === "NoParseText")
      .map((token) => token.image)
      .join(""),
    "[i]text[/i]",
  );
  assert.equal(
    result.tokens.filter((token) => token.tokenType.name === "TagStart").length,
    0,
  );
});
...
for (const source of [
  ...
  test(`native tokens and ranges retain attribute/unknown syntax: ${source}`, () => {
    ...
    for (const token of result.tokens) {
      assert.equal(
        source.slice(token.startOffset, (token.endOffset ?? -1) + 1),
        token.image,
      );
    }
  });
...
test("resource limits stop excessive depth, node count, attributes and UTF-8 bytes", () => {
  assert.ok(
    parseSteamBbcodeSyntax("[b][i]text[/i][/b]", {
      maxNestingDepth: 1,
    }).diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_MAX_NESTING_DEPTH_EXCEEDED",
    ),
  );
  assert.ok(
    parseSteamBbcodeSyntax("[*][*][*]", { maxNodeCount: 2 }).diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_MAX_NODE_COUNT_EXCEEDED",
    ),
  );
  assert.ok(
    parseSteamBbcodeSyntax("[url=abcdef]X[/url]", {
      maxAttributeBytes: 3,
    }).diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_MAX_ATTRIBUTE_BYTES_EXCEEDED",
    ),
  );
  assert.ok(
    parseSteamBbcodeSyntax("😀", { maxInputBytes: 3 }).diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_MAX_INPUT_BYTES_EXCEEDED",
    ),
  );
  assert.deepEqual(
    parseSteamBbcodeSyntax("😀", { maxInputBytes: 4 }).diagnostics,
    [],
  );
});
...
test("supported recursion depth is executable and the next level stops with a diagnostic", () => {
  ...
  const exceeded = parseSteamBbcodeSyntax("[b]" + atLimit + "[/b]", {
    maxNestingDepth: 256,
  });
  assert.equal(exceeded.cst, undefined);
  assert.ok(
    exceeded.diagnostics.some(
      (diagnostic) => diagnostic.code === "STEAM_MAX_NESTING_DEPTH_EXCEEDED",
    ),
  );
});
...
test("attribute quota uses UTF-8 bytes for complete and unfinished headers", () => {
  assert.deepEqual(
    parseSteamBbcodeSyntax("[url=é]X[/url]", { maxAttributeBytes: 3 })
      .diagnostics,
    [],
  );
  for (const source of ["[url=é]X[/url]", "[url=é"]) {
    assert.ok(
      parseSteamBbcodeSyntax(source, { maxAttributeBytes: 2 }).diagnostics.some(
        (diagnostic) =>
          diagnostic.code === "STEAM_MAX_ATTRIBUTE_BYTES_EXCEEDED",
      ),
    );
  }
});
...
test("diagnostic unist spans end after LF, CRLF, CR and surrogate pairs", () => {
  for (const ending of ["\n", "\r\n", "\r"]) {
    ...
    const diagnostic = parseSteamBbcodeSyntax(source).diagnostics.find(
      (item) => item.code === "STEAM_UNCLOSED_TAG",
    );
...
test("JavaScript callers receive validation errors for wrong inputs and invalid limits", () => {
  for (const args of [
    ...
    assert.throws(
      () => Reflect.apply(parseSteamBbcodeSyntax, undefined, args),
...
test("bounded hostile text remains lossless across maintained generated/shrunk cases", () => {
  ...
  fc.assert(
    fc.property(fc.array(fragment, { maxLength: 80 }), (fragments) => {
      ...
      assert.ok(
        result.cst,
        "Every bounded input has a syntactic representation.",
      );
      assert.equal(
        result.diagnostics.some(
          (diagnostic) =>
            diagnostic.code === "STEAM_LEXICAL_ERROR" ||
            diagnostic.code === "STEAM_SYNTAX_ERROR",
        ),
        false,
      );
    }),
