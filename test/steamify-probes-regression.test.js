...
const history = JSON.parse(
  await readFile(
    new URL("./fixtures/steamify/historical-results.json", import.meta.url),
    "utf8",
  ),
);
const probes = history.checks.filter((check) =>
  check.name.startsWith("probe: "),
);
...
for (const probe of probes) {
  test(`Steamify ${probe.name}`, () => {
    ...
    const tree = fromMarkdown(result.value, {
      extensions: [gfm()],
    ...
    const text = all
      ...
      .join("");
    switch (probe.name) {
      case "probe: nested unordered list":
      case "probe: nested ordered list": {
      ...
      case "probe: image":
        assert.ok(
          all.some(
            (node) =>
              node.type === "image" &&
              node.url === "https://example.com/badge.png",
          ),
        );
        break;
      ...
      case "probe: bare URL tag":
        assert.ok(
          all.some(
            (node) =>
              node.type === "link" && node.url === "https://example.com",
          ),
        );
      ...
      case "probe: multiline bold": {
        ...
        assert.equal(
          strong.children[0]?.type === "text" ? strong.children[0].value : "",
          "First\nsecond",
        );
        break;
      }
      case "probe: underline":
        ...
        assert.ok(
          result.diagnostics.some(
            (diagnostic) =>
              diagnostic.code === "STEAM_UNDERLINE_LOWERED_TO_TEXT" &&
        ...
        assert.ok(
          result.coverage.constructs.some(
            (outcome) =>
              outcome.constructId === "steam.bbcode.u" &&
              outcome.fidelity === "lossy",
          ),
        );
        break;
      case "probe: spoiler":
        assert.ok(
          tree.children.some(
            (node) => node.type === "html" && node.value.includes("<details>"),
          ),
        );
        assert.equal(text, "Secret");
        assert.ok(
          result.diagnostics.some(
            (diagnostic) => diagnostic.fidelity === "approximate",
          ),
        );
        break;
      case "probe: noparse":
        ...
        assert.equal(
          all.some((node) => node.type === "strong"),
          false,
        );
        break;
      case "probe: table":
      ...
      case "probe: literal Markdown emphasis":
        ...
        assert.equal(
          all.some((node) => node.type === "emphasis"),
          false,
        );
        break;
      case "probe: inline code containing backticks":
      case "probe: block code containing fences": {
        const code = all.find((node) => node.type === "code");
        assert.ok(code?.type === "code");
        assert.equal(
          code.value,
          probe.name === "probe: inline code containing backticks"
            ? "a`b"
            : "\n```\ntext\n",
        );
        break;
      ...
      case "probe: nested blockquotes":
        assert.equal(
          all.filter((node) => node.type === "blockquote").length,
          2,
        );
        break;
      case "probe: horizontal rule":
        assert.equal(tree.children[0]?.type, "thematicBreak");
        break;
      case "probe: strikethrough":
        assert.ok(all.some((node) => node.type === "delete"));
        assert.equal(text, "Old");
        break;
      default:
        assert.fail(
          `The frozen historical probe has no independent oracle: ${probe.name}`,
        );
    }
  });
}
