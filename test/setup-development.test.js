...
function fixture(t) {
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({packageManager: 'npm@12.0.2'}));
  ...
  const execute = (command, args, cwd) => {
    ...
    const stdout = args.includes('--version') ? '12.0.2\n' : args.includes('-c') ? '3.14.7\n' : '';
...
test('standalone setup installs exactly the locked graphs and isolated prose tools in a path with spaces', t => {
  ...
  assert.ok(installs.every(call => call.command === process.execPath && call.root === f.root));
  assert.ok(f.calls.some(call => call.command === f.python && call.args.join(' ') === '-I -m venv .venv'));
  const pip = f.calls.find(call => call.args.includes('pip'));
  assert.ok(pip);
  assert.equal(pip.command, join(f.root, '.venv', process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python'));
  assert.deepEqual(pip.args, ['-B', '-m', 'pip', 'install', '--require-hashes', '--only-binary=:all:', '-r', 'tooling/prose/requirements.txt']);
});
...
test('mismatched npm and Python versions stop before installations', t => {
  for (const prerequisite of ['npm', 'Python']) {
    ...
    f.execute = (command, args, root) => {
      ...
      return args.includes(prerequisite === 'npm' ? '--version' : '-c') ? {...result, stdout: '0.0.0\n'} : result;
    };
    assert.throws(() => setupDevelopment(f), new RegExp(prerequisite));
    assert.ok(f.calls.every(call => !call.args.includes('ci') && !call.args.includes('venv')));
  }
});

test('missing Python and interrupted probes retain the actual failure and do not install', t => {
  for (const result of [{status: null, stdout: '', stderr: '', error: new Error('missing Python executable')},
    {status: null, stdout: '', stderr: '', signal: 'SIGTERM'}]) {
    const f = fixture(t);
    const execute = f.execute;
    f.execute = (command, args, root) => command === f.python ? result : execute(command, args, root);
    assert.throws(() => setupDevelopment(f), /missing Python executable|SIGTERM/);
...
test('a failed install stops the sequence and preserves its exit status in the diagnostic', t => {
  ...
  f.execute = (command, args, root) => {
    const result = execute(command, args, root);
    return args.includes('ci') ? {...result, status: 7, stderr: 'fixture install failed'} : result;
  };
  ...
  assert.ok(f.calls.every(call => !call.args.includes('venv') && !call.args.includes('pip')));
});
...
test('an existing environment is validated rather than overwritten', t => {
  ...
  f.execute = (command, args, root) => {
    const result = execute(command, args, root);
    return command.includes('.venv') ? {...result, stdout: '3.13.0\n'} : result;
  };
