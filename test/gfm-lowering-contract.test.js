...
test("paragraph spoilers separate surrounding flow and escape their literal content", () => {
    ...
    children: [
      {
        ...
        children: [
          ...
          {
            type: "steamSpoiler",
            children: [{ type: "text", value: "<img src=x>\r\n*literal*" }],
          },
  ...
  assert.deepEqual(
    rendered.children.map((n) => n.type),
    ["paragraph", "html", "paragraph", "html", "paragraph"],
  );
  const content = rendered.children[2];
  assert.ok(content?.type === "paragraph");
  assert.deepEqual(
    content.children.map((n) => (n.type === "text" ? n.value : n.type)),
    ["<img src=x>\n*literal*"],
  );
  assert.deepEqual(
    result.diagnostics.map((d) => [d.code, d.fidelity, d.sourceSpan]),
    [["STEAM_SPOILER_LOWERED_TO_DETAILS", "approximate", undefined]],
  );
  assert.deepEqual(source, original);
});

test("a spoiler inside phrasing retains text and reports lost concealment without block HTML", () => {
  const result = lowerSteamMdastToGfm(
    sourceResult({
      type: "root",
      children: [
        {
              ...
              children: [
                {
                  type: "steamSpoiler",
                  children: [
                    { type: "steamNoParse", value: "[b]secret[/b]\rtext" },
                  ],
                },
              ],
            },
    ...
    children: [
      {
        ...
        children: [
          {
            type: "strong",
            children: [{ type: "text", value: "[b]secret[/b]\ntext" }],
          },
        ],
      },
    ],
  ...
  assert.deepEqual(
    result.diagnostics.map((d) => [d.code, d.fidelity]),
    [["STEAM_SPOILER_LOWERED_TO_TEXT", "lossy"]],
  );
...
test("embedded media retains an authored label through the native link contract", () => {
      ...
      children: [
        {
          ...
          mediaKind: "video",
          source: "https://example.org/clip.mp4",
          children: [
            {
              type: "emphasis",
              children: [{ type: "text", value: "Play clip" }],
            },
          ],
        },
      ],
  ...
  assert.deepEqual(
    result.diagnostics.map((d) => [d.code, d.fidelity]),
    [["STEAM_MEDIA_EMBED_LOWERED_TO_LINK", "lossy"]],
  );
...
test("typed root fragments lower their Steam children while retaining native container fields", () => {
  ...
  const fragments = [
    ...
    },
    {
      type: "tableCell",
      children: [
        { type: "steamUnderline", children: [{ type: "text", value: "Cell" }] },
      ],
    },
  ...
  for (const [index, fragment] of fragments.entries()) {
    ...
    assert.deepEqual(lowerSteamMdastToGfm(source).value, {
      type: "root",
      children: [expected[index]],
    });
...
test("a standalone phrasing spoiler creates no empty surrounding paragraphs", () => {
  ...
  assert.deepEqual(
    result.value.children.map((node) => node.type),
    ["html", "paragraph", "html"],
  );
});
...
test("positioned extension diagnostics retain their exact source occurrence", () => {
  ...
  assert.deepEqual(
    result.coverage.constructs,
    [],
    "missing source accounting must not be fabricated",
  );
});
