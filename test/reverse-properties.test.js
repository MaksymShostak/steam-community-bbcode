...
test("reverse bounded arbitrary Markdown returns a result with complete source accounting or an input failure", () => {
  ...
  fc.assert(
    fc.property(fc.array(fragment, { maxLength: 70 }), (parts) => {
      const result = gfmToSteamCommunityBbcode(parts.join(""), {
        resourceLimits: {
          maxNodeCount: 128,
          maxNestingDepth: 16,
      ...
      if (
        result.diagnostics.some(
          (item) => item.scope === "input" && item.severity === "error",
        )
      ) {
        ...
        assert.ok(
          result.coverage.sourceNodes.some((node) => node.nodeType === "root"),
        );
    }),
