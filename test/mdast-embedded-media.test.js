...
test("a YouTube preview retains its identifier and layout, then becomes a diagnosed link", () => {
  ...
  assert.equal(
    media.mediaKind === "youtube" ? media.layout : undefined,
    "leftthumb",
  );
  ...
  assert.ok(
    result.diagnostics.some(
      (d) =>
        d.code === "STEAM_MEDIA_EMBED_LOWERED_TO_LINK" &&
...
test("video parameters preserve source and poster URLs without claiming Steam playback", () => {
  const source =
    '[video mp4="https://example.org/clip.mp4?a=1&b=2" poster=https://example.org/poster.png autoplay=0][/video]';
  const semantic = steamCommunityBbcodeToMdast(source, {
    profile: "ugc-description",
  });
  const media = semantic.value.children[0];
  assert.ok(
    media?.type === "steamEmbeddedMedia" && media.mediaKind === "video",
  );
...
for (const source of [
  ...
  test(`an unqualified media form preserves all source: ${source}`, () => {
    ...
    assert.ok(
      result.diagnostics.some((d) => d.code === "STEAM_CONSTRUCT_PRESERVED"),
    );
  });
}
...
test("a video without optional presentation retains only its source and uses the fallback link label", () => {
  ...
  assert.deepEqual(
    link.children.map((n) => (n.type === "text" ? n.value : n.type)),
    ["Video"],
  );
  assert.ok(
    result.diagnostics.some(
