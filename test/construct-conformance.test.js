...
for (const profile of steamDialectProfileIds) {
  for (const fixture of constructCases)
    test(`construct conformance: ${profile}/${fixture.id}`, () => {
      executeConstructCase(fixture, profile);
    });
}

test("every source-observable registry entry has an authored executable case", () => {
  const sourceIds = steamConstructDefinitions
    .filter((d) => !("observation" in d && d.observation === "contextOnly"))
    .map((d) => d.id);
  assert.deepEqual(
    new Set(constructCases.flatMap((c) => c.constructIds)),
    new Set(sourceIds),
  );
});
