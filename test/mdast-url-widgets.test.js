...
for (const [kind, url] of [
  ...
  [
    "inventory-widget",
    "https://steamcommunity.com/profiles/123/inventory#440_22_123",
  ],
  ...
  test(`a plain ${kind} URL retains its link destination and accounts for widget loss`, () => {
    ...
    const outcome = semantic.coverage.constructs.find(
      (c) => c.constructId === `steam.renderer.${kind}`,
    );
    assert.ok(outcome);
    assert.equal(
      source.slice(
        outcome.sourceSpan.start.offset,
        outcome.sourceSpan.end.offset,
      ),
      url,
    );
    ...
    assert.ok(
      result.diagnostics.some(
        (d) =>
          d.code === "STEAM_URL_WIDGET_LOWERED_TO_LINK" &&
          d.fidelity === "approximate",
      ),
    );
  });
}
...
test("opaque and linked text do not acquire nested URL widgets; deceptive hosts remain ordinary text", () => {
  ...
  for (const source of [
    `[noparse]${url}[/noparse]`,
    `[code]${url}[/code]`,
    `[url=https://example.org]${url}[/url]`,
    "https://store.steampowered.com.evil.invalid/app/457140/",
    "https://www.youtube.com/ordinary-page",
  ]) {
    const result = steamCommunityBbcodeToMdast(source);
    assert.ok(
      !result.coverage.constructs.some((c) =>
        c.constructId.startsWith("steam.renderer."),
      ),
    );
  }
});
...
test("widget recognition requires the recorded host, complete path and identifier family", () => {
  for (const url of [
    ...
    "https://youtube.com/other?v=abc",
    "https://youtu.be/",
    "https://youtu.be/a/b",
    "https://youtu.be/a.b",
    "https://youtube.com.evil.invalid/watch?v=abc",
    "https://user@youtube.com/watch?v=abc",
    "https://:secret@youtube.com/watch?v=abc",
    "https://store.steampowered.com/not/app/123/",
    ...
    "https://vimeo.com/123abc",
    "https://vimeo.com/extra/123",
    "https://vimeo.com/123/extra",
    "https://sketchfab.com/3d-models/",
    "https://sketchfab.com/3d-models/a/b",
    "https://sketchfab.com/not/3d-models/a",
    "ftp://vimeo.com/123",
    "//vimeo.com/123",
    "vimeo.com/123",
    "mailto:vimeo.com/123",
  ]) {
    ...
    for (const link of links) {
      ...
      const position = {
        start: { line: 1, column: offset + 1, offset },
          ...
          line: 1,
          column: offset + link.url.length + 1,
          offset: offset + link.url.length,
        },
      };
      assert.deepEqual(link.position, position);
      assert.deepEqual(link.children, [
        { type: "text", value: link.url, position },
      ]);
    }
  }
});
