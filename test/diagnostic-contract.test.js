...
} from "../src/diagnostics/diagnostic-codes.js";
...
test("diagnostic catalogues have unique nonempty identifiers and include parser outcomes", () => {
  ...
  assert.ok(
    steamParseDiagnosticCodes.every((code) =>
      conversionDiagnosticCodes.includes(code),
    ),
  );
});
...
test("conformance diagnostics identify an outcome and explain it to the caller", () => {
  ...
  const conversions = [
    ...constructCases.flatMap((fixture) => [
      steamCommunityBbcodeToGfm(fixture.source),
      steamCommunityBbcodeToGfm(fixture.malformed),
    ]),
    ...constructCases.flatMap((fixture) => [
      steamCommunityBbcodeToMdast(fixture.source),
      steamCommunityBbcodeToMdast(fixture.malformed),
    ]),
  ...
  for (const diagnostic of diagnostics) {
    assert.ok(
      knownCodes.has(diagnostic.code),
      `Unknown emitted diagnostic: ${diagnostic.code}`,
    );
    assert.notEqual(
    ...
    assert.ok(
      ["exact", "equivalent", "approximate", "lossy", "unsupported"].includes(
        diagnostic.fidelity,
      ),
    );
    ...
    if (diagnostic.scope === "construct") {
      assert.ok(
        diagnostic.constructId === "steam.bbcode.unknown" ||
          steamConstructIds.includes(diagnostic.constructId),
      );
      assert.ok(diagnostic.sourceSpan);
    }
    if (diagnostic.scope === "gfm-node")
      assert.notEqual(diagnostic.nodeType, "");
  }
  for (const result of conversions) {
    for (const outcome of result.coverage.constructs) {
      assert.ok(
        outcome.constructId === "steam.bbcode.unknown" ||
          steamConstructIds.includes(outcome.constructId),
      );
      assert.ok(
        ["exact", "equivalent", "approximate", "lossy", "unsupported"].includes(
          outcome.fidelity,
...
test("resource exhaustion is an input error and malformed syntax is a warning", () => {
  for (const convert of [
    ...
    const inputDiagnostic = malformed.diagnostics.find(
      (d) => d.scope === "input",
    );
    assert.ok(inputDiagnostic);
    assert.deepEqual(
      [
        inputDiagnostic.code,
        inputDiagnostic.severity,
        inputDiagnostic.fidelity,
      ],
