...
const presentation = readFileSync(
  new URL("fixtures/github-rendering/presentation.html", import.meta.url),
  "utf8",
);
const literalAutolinks = readFileSync(
  new URL("fixtures/github-rendering/literal-autolinks.html", import.meta.url),
  "utf8",
);
const flowWhitespace = readFileSync(
  new URL("fixtures/github-rendering/flow-whitespace.html", import.meta.url),
  "utf8",
);
...
function runProbe(scenario, responses) {
  ...
  try {
    cpSync(join(packageDirectory, "src"), join(sandbox, "src"), {
      recursive: true,
    });
    cpSync(join(packageDirectory, "scripts"), join(sandbox, "scripts"), {
      recursive: true,
    });
    ...
    assert.equal(
      dirname(sandbox),
      resolve(artifacts),
      "cleanup must stay within the task artifact directory",
    );
...
test("presentation qualification permits transport line endings and trailing newlines", () => {
  const result = runProbe("presentation", [
...
const wrongResponses = [
  ...
  [
    "presentation",
    "lost insertion markup",
    presentation.replace("<ins>Inserted</ins>", "Inserted"),
  ],
  [
    "presentation",
    "lost spoiler structure",
    presentation.replace("<details>", "<div>").replace("</details>", "</div>"),
  ],
  [
    "presentation",
    "lost table structure",
    presentation
      .replace('<table role="table">', "<div>")
      .replace("</table>", "</div>"),
  ],
  [
    "presentation",
    "active script markup",
  ...
  [
    "literal-autolinks",
    "new active literal link",
    literalAutolinks.replace(
...
for (const [scenario, name, html] of wrongResponses) {
  test(`renderer qualification rejects HTTP 200 with ${name}`, () => {
    ...
    assert.notEqual(
      result.status,
      0,
      "incorrect successful HTML must fail qualification",
    );
...
test("flow qualification preserves literal padding despite transport normalization", () => {
  const result = runProbe("flow-whitespace", [
    flowWhitespace.replace("  [sd]  [Fixed]  ", "[sd]  [Fixed]"),
    flowWhitespace,
  ]);
  assert.equal(result.error, undefined);
