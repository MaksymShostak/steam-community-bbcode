...
test('actual publication gate accepts exact identity and rejects mismatched inputs', () => {
  ...
  for (const job of jobs.items) {
    ...
    for (const step of steps.items) {
      ...
      if (step.get('name') === 'Check publication eligibility') command = step.get('run');
    }
  }
  ...
  const candidate = {schemaVersion: 2,
    ...
    source: {dirty: false, commit: sha}, checks: {installedConsumers: 'PASS', productionAudit: 'PASS'},
    release: {repository, repositoryId: '1234', ref: 'refs/heads/main', workflowRef, workflowSha: sha,
      runId: '5678', runAttempt: '1', tag: 'next',
      qualification: {runtimeAndMutation: 'success', controls: 'success', security: 'success'}},
  };
  const validEnvironment = {GITHUB_REPOSITORY: repository, GITHUB_REPOSITORY_ID: '1234', GITHUB_REF: 'refs/heads/main',
    GITHUB_EVENT_NAME: 'workflow_dispatch', GITHUB_WORKFLOW_REF: workflowRef, GITHUB_WORKFLOW_SHA: sha,
    GITHUB_SHA: sha, GITHUB_RUN_ID: '5678', GITHUB_RUN_ATTEMPT: '1',
    RELEASE_VERSION: '1.0.0-rc.1', RELEASE_TAG: 'next', CANDIDATE_ARTIFACT_ID: '4321'};
    ...
    function exercise(value, overrides, accepted) {
      ...
      const result = spawnSync(bash, ['--noprofile', '--norc', '-eo', 'pipefail', '-c', command], {
        ...
        env: {...process.env, ...validEnvironment,
          CANDIDATE_SHA256: createHash('sha256').update(json).digest('hex'), ...overrides},
      });
      assert.equal(result.error, undefined);
      assert.equal(result.status === 0, accepted, `${JSON.stringify(overrides)}\n${result.stderr}`);
    }
    exercise(candidate, {}, true);
    for (const [key, value] of Object.entries({GITHUB_REPOSITORY: 'wrong/repository', GITHUB_REPOSITORY_ID: '9876',
      GITHUB_REF: 'refs/heads/other', GITHUB_EVENT_NAME: 'push', GITHUB_WORKFLOW_REF: 'other.yml@refs/heads/main',
      GITHUB_WORKFLOW_SHA: 'b'.repeat(40), GITHUB_SHA: 'c'.repeat(40), GITHUB_RUN_ID: '8765', GITHUB_RUN_ATTEMPT: '2',
      RELEASE_VERSION: '1.0.0-rc.2', RELEASE_TAG: 'latest', CANDIDATE_ARTIFACT_ID: '', CANDIDATE_SHA256: '0'.repeat(64)})) {
      exercise(candidate, {[key]: value}, false);
    }
    exercise({...candidate, package: {...candidate.package, private: true}}, {}, false);
    exercise({...candidate, package: {...candidate.package, name: 'other-package'}}, {}, false);
    exercise({...candidate, source: {...candidate.source, dirty: true}}, {}, false);
    exercise({...candidate, source: {...candidate.source, commit: 'b'.repeat(40)}}, {}, false);
    for (const key of ['runtimeAndMutation', 'controls', 'security']) {
      for (const result of ['failure', 'skipped', 'cancelled', '']) {
        exercise({...candidate, release: {...candidate.release,
          qualification: {...candidate.release.qualification, [key]: result}}}, {}, false);
      }
    }
...
test('release qualification reuses all same-revision runtime, mutation, control and security workflows', () => {
  const {document} = readWorkflow('steam-community-bbcode-release');
  /** @type {[string, string][]} */
  const required = [['qualification', 'steam-community-bbcode'], ['controls', 'sdlc-control-tests'], ['security', 'codeql']];
  for (const [job, file] of required) {
    assert.equal(document.getIn(['jobs', job, 'uses']), `./.github/workflows/${file}.yml`);
...
test('release uses root consumers and explicit immutable checkout without privileged project execution', () => {
  ...
  assert.equal(document.getIn(['on', 'workflow_dispatch', 'inputs', 'publish', 'default']), false);
  ...
  assert.doesNotMatch(publish.toString(), /actions\/checkout|npm ci|npm run|node scripts|working-directory: tools/u);
  for (const job of ['publish', 'verify_registry']) {
    const steps = document.getIn(['jobs', job, 'steps'], true);
    assert.ok(isSeq(steps));
    const download = steps.items.find(step => isMap(step) && String(step.get('uses')).startsWith('actions/download-artifact@'));
    assert.ok(isMap(download));
    assert.equal(download.getIn(['with', 'artifact-ids']), '${{ needs.candidate.outputs.artifact_id }}');
