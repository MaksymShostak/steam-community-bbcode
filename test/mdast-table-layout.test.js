...
test("known table layout is retained as metadata and its target loss is diagnosed", () => {
  const source =
    '[table equalcells="1" noborder=1][tr][th]H[/th][/tr][tr][td]C[/td][/tr][/table]';
  ...
  assert.deepEqual(table.data?.["steamTableLayout"], {
    equalcells: true,
    noborder: true,
  });
  ...
  assert.ok(
    converted.diagnostics.some(
      (diagnostic) =>
        diagnostic.code === "STEAM_TABLE_LAYOUT_OMITTED" &&
...
test("explicitly disabled table layout has no omitted presentation to diagnose", () => {
  ...
  assert.ok(
    !result.diagnostics.some((d) => d.code === "STEAM_TABLE_LAYOUT_OMITTED"),
  );
  const target = fromMarkdown(result.value, {
    extensions: [gfm()],
    mdastExtensions: [gfmFromMarkdown()],
  });
