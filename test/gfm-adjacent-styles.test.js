  ...
  function visit(nodes, marks) {
    for (const node of nodes) {
      if (node.type === "text") {
        ...
        visit(
          node.children,
          marks | { strong: 1, emphasis: 2, delete: 4 }[node.type],
        );
      }
    }
  }
...
test("GFM formatting preserves equivalent adjacent style order", () => {
  ...
  assert.deepEqual(characterStyles(result.value), [
    ["a", 3],
    ["b", 3],
  ]);
  assert.deepEqual(result.diagnostics, []);
  assert.ok(
    result.coverage.constructs.every((outcome) => outcome.fidelity === "exact"),
  );
});
