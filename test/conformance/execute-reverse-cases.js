...
export function executeReverseCase(fixture) {
  ...
  for (const type of fixture.unsupported)
    assert.ok(
      nodes.some((node) => node.type === type),
      `${fixture.id}: native source precondition ${type}`,
    );
  ...
  assert.deepEqual(
    syntax.diagnostics,
    [],
    `${fixture.id}: valid target syntax`,
  );
  ...
  assert.deepEqual(
    result.unsupportedSourceNodes.map((item) => item.nodeType).sort(),
    [...fixture.unsupported].sort(),
  );
  ...
  assert.deepEqual(
    result.coverage.sourceNodes
      .map((node) => JSON.stringify([node.nodeType, node.sourceSpan]))
      .sort(),
    nodes.map((node) => JSON.stringify([node.type, node.position])).sort(),
    `${fixture.id}: every native source node accounted for once`,
  );
  if (fixture.unsupported.length) {
    const literal = syntax.children
      .map((node) => {
        assert.ok(
          node.type === "steamText" ||
            (node.type === "steamOpaqueTag" && node.tagName === "noparse"),
          `${fixture.id}: no active markup in preserved source`,
        );
        return node.value;
      })
      .join("");
    assert.equal(
      literal,
      fixture.source,
      `${fixture.id}: complete literal source preserved`,
    );
  } else {
    assert.ok(fixture.expectedSteam !== undefined && fixture.expectedGfm);
    assert.equal(
      result.value,
      fixture.expectedSteam,
      `${fixture.id}: authored target syntax`,
    );
    assert.deepEqual(
      semanticProjection(
        parseGfm(steamCommunityBbcodeToGfm(result.value).value),
      ),
      fixture.expectedGfm,
      `${fixture.id}: authored semantic tree`,
    );
    assert.ok(
      result.coverage.sourceNodes.every(
        (node) => node.fidelity === "equivalent",
      ),
    );
  }
  return {
    ...
    unsupportedSourceNodeTypes: fixture.unsupported,
