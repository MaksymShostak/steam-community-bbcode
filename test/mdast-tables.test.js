...
test("a rectangular Steam table uses native rows and cells with nested inline semantics", () => {
  ...
  assert.equal(
    table.children[1]?.children[1]?.children[0]?.type,
    "steamUnderline",
  );
  assert.equal(
    result.coverage.constructs.filter(
      (outcome) => outcome.constructId === "steam.bbcode.tr",
    ).length,
    2,
  );
  assert.equal(
    result.coverage.constructs.filter(
      (outcome) => outcome.constructId === "steam.bbcode.td",
    ).length,
    2,
  );
});
