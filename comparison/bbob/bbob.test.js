...
function structure(value) {
  if (Array.isArray(value)) {
    ...
    for (const child of value) {
      ...
      if (typeof previous === "string" && typeof projected === "string")
        result[result.length - 1] = previous + projected;
  ...
  if (
    ...
    return {
      tag: value.tag,
      attrs: value.attrs,
      content: structure(value.content),
    };
  }
  return value;
}
  ...
  assert.deepEqual(structure(parse("[B]bold [i]italic[/i][/B]", options)), [
    {
      tag: "B",
      attrs: {},
      content: ["bold ", { tag: "i", attrs: {}, content: ["italic"] }],
    },
  ]);
});
...
for (const tag of ["list", "olist"]) {
  test(`nested ${tag} keeps child list inside the parent item interval`, () => {
    assert.deepEqual(
      structure(
        parse(
          `[${tag}][*]Parent[${tag}][*]Child[/${tag}][*]Next[/${tag}]`,
          options,
        ),
      ),
      [
        {
          ...
          content: [
            ...
            "Parent",
            {
              tag,
              attrs: {},
              content: [{ tag: "*", attrs: {}, content: [] }, "Child"],
            },
            { tag: "*", attrs: {}, content: [] },
            "Next",
          ],
...
for (const [source, tag, attrs] of [
  ...
  [
    '[url="https://example.com/a?x=1"]text[/url]',
    "url",
    { "https://example.com/a?x=1": "https://example.com/a?x=1" },
  ],
  [
    "[table noborder=1 equalcells=1]text[/table]",
    "table",
    { noborder: "1", equalcells: "1" },
  ],
  [
    "[video mp4=https://example.com/a.mp4 poster=https://example.com/a.png]text[/video]",
    "video",
    { mp4: "https://example.com/a.mp4", poster: "https://example.com/a.png" },
  ],
  [
    "[previewimg=420;sizeFull,floatLeft;example.png]text[/previewimg]",
    "previewimg",
    {
      "420;sizeFull,floatLeft;example.png":
        "420;sizeFull,floatLeft;example.png",
    },
  ],
]) {
  test(`Steam attributes remain available: ${String(tag)}`, () => {
    ...
    assert.deepEqual(
      structure(parse(/** @type {string} */ (source), options)),
...
test("unknown tags retain the exact source range including attribute spelling", () => {
  ...
  const node = tree.find(
    (item) => typeof item === "object" && item.tag === "futuretag",
  );
  assert.ok(node?.start && node.end);
  assert.equal(
    source.slice(node.start.from, node.end.to),
    '[futuretag  foo="bar baz"]text[/futuretag]',
  );
});
