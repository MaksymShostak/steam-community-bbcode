...
} from "./steam/registry-identifiers.js";
...
async function readSource(path, maxInputBytes) {
  const stream =
    path === undefined || path === "-" ? process.stdin : createReadStream(path);
  /** @type {Buffer[]} */
  const chunks = [];
  let byteLength = 0;
  ...
  for await (const chunk of input) {
    if (!Buffer.isBuffer(chunk))
      throw new TypeError("Expected raw input bytes.");
    byteLength += chunk.length;
    if (byteLength > maxInputBytes)
      throw new RangeError(
        `CLI input exceeds the ${maxInputBytes} byte limit.`,
      );
    chunks.push(chunk);
  }
...
async function main() {
  ...
  if (command === "coverage") {
    if (
      ...
      Object.keys(values).some((key) => key !== "format") ||
      (values.format !== undefined && values.format !== "json")
    )
      throw new TypeError(
        "Use coverage --format=json without conversion options or an input file.",
      );
    writeFileSync(
      1,
  ...
  if (command !== "to-gfm" && command !== "to-steam")
    throw new TypeError("Expected to-gfm or to-steam; use --help.");
  if (positionals.length > 2)
    throw new TypeError("Expected at most one input file.");
  const format = values.format ?? "text";
  if (format !== "text" && format !== "json")
    throw new TypeError("Expected --format=text or --format=json.");
  const profile = steamDialectProfileIds.find(
    (id) => id === (values.profile ?? defaultSteamDialectProfile),
  );
  if (profile === undefined) throw new TypeError("Unknown Steam profile.");
  if (command === "to-steam" && values.profile !== undefined)
    throw new TypeError("--profile applies only to to-gfm.");
  ...
  if (
    failurePolicy === "approximate" ||
    failurePolicy === "lossy" ||
    failurePolicy === "unsupported"
  )
    failureRank = fidelityRanks[failurePolicy];
  else if (failurePolicy !== "none")
    throw new TypeError(
      "Expected --fail-on=none, approximate, lossy or unsupported.",
    );
  const maxInputBytes =
    values["max-input-bytes"] === undefined
      ? defaultSteamParseResourceLimits.maxInputBytes
      : Number(values["max-input-bytes"]);
  if (!Number.isSafeInteger(maxInputBytes) || maxInputBytes < 1)
    throw new RangeError("--max-input-bytes must be a positive safe integer.");
  const source = await readSource(path, maxInputBytes);
  const result =
    command === "to-gfm"
      ? steamCommunityBbcodeToGfm(source, {
          profile,
          resourceLimits: { maxInputBytes },
        })
      : gfmToSteamCommunityBbcode(source, {
          resourceLimits: { maxInputBytes },
        });
  ...
  return result.diagnostics.some(
    (diagnostic) =>
      ...
      fidelityRanks[diagnostic.fidelity] >= failureRank,
  )
    ? 1
    : 0;
}
...
try {
  ...
  try {
    writeFileSync(
      2,
      `${JSON.stringify({ code: "CLI_ERROR", message: error instanceof Error ? error.message : String(error) })}\n`,
    );
  } catch {
