...
export function steamCommunityBbcodeToGfm(source, options = {}) {
  if (typeof source !== "string")
    throw new TypeError("The GFM conversion source must be a string.");
  const result = lowerSteamMdastToGfm(
    steamCommunityBbcodeToMdast(source, options),
  );
  ...
  const value = toMarkdown(result.value, {
    extensions: [gfmToMarkdown()],
    fences: true,
    emphasis: "_",
    unsafe: [...whitespace].map((character) => ({
      character,
      inConstruct: "strikethrough",
    })),
  });
  return {
    ...result,
    ...
    diagnostics: [
      ...result.diagnostics,
      ...rendererAutolinkDiagnostics(result.value),
    ],
  };
}
