...
export function gfmParagraphs(paragraph) {
  ...
  const projection = runs
    .map((run) => (run.node.type === "text" ? run.node.value : "\uFFFC"))
    .join("");
  ...
  for (const span of spans) {
    ...
    const contents = [];
    for (
      let run = runs[firstRun];
      run && run.end <= span.start;
      run = runs[++firstRun]
    ) {
    ...
    for (let index = firstRun; index < runs.length; index++) {
      ...
      const node =
        run.node.type === "text"
          ? {
              ...run.node,
              value: run.node.value.slice(
                Math.max(0, span.start - run.start),
                span.end - run.start,
              ),
            }
          : run.node;
  ...
  function visit(nodes, ancestors) {
    for (const node of nodes) {
      if ("children" in node && node.children.length) {
        const repeated =
          isPresentation(node) &&
          ancestors.some((parent) => parent.type === node.type);
        visit(node.children, repeated ? ancestors : [...ancestors, node]);
      } else if (
        ("children" in node && isPresentation(node)) ||
        ...
        runs.push({
          node,
          ancestors: orderedPresentation(ancestors),
          start: offset,
          end: offset + length,
        });
        offset += length;
...
function assemble(runs) {
  ...
  for (const { node, ancestors } of runs) {
    ...
    while (common < preceding.length && common < ancestors.length) {
      ...
      if (
        left !== right &&
        !(left && right && isPresentation(left) && left.type === right.type)
      )
        break;
      common++;
    }
...
function orderedPresentation(ancestors) {
  ...
  function flush() {
    ...
    result.push(
      ...marks.sort((a, b) => presentationOrder(a) - presentationOrder(b)),
    );
    marks = [];
  }
