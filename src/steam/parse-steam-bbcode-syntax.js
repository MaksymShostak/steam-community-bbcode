...
import {
  defaultSteamParseResourceLimits,
  resolveSteamParseResourceLimits,
...
const CodeStart = createToken({
  name: "CodeStart",
  pattern: /\[code(?=[\s=\]])/i,
  push_mode: "codeAttributes",
});
const NoParseStart = createToken({
  name: "NoParseStart",
  pattern: /\[noparse(?=[\s=\]])/i,
  push_mode: "noParseAttributes",
});
const HorizontalRuleStart = createToken({
  name: "HorizontalRuleStart",
  pattern: /\[hr(?=[\s=\]])/i,
  push_mode: "attributes",
});
...
const LiteralText = createToken({
  name: "LiteralText",
  pattern: /[^[]+/,
  line_breaks: true,
});
const LiteralBracket = createToken({ name: "LiteralBracket", pattern: /\[/ });
const HeaderEnd = createToken({
  name: "HeaderEnd",
  pattern: /\]/,
  pop_mode: true,
});
const CodeHeaderEnd = createToken({
  name: "CodeHeaderEnd",
  pattern: /\]/,
  categories: [HeaderEnd],
  pop_mode: true,
  push_mode: "codeBody",
});
const NoParseHeaderEnd = createToken({
  name: "NoParseHeaderEnd",
  pattern: /\]/,
  categories: [HeaderEnd],
  pop_mode: true,
  push_mode: "noParseBody",
});
const AttributeSpace = createToken({
  name: "AttributeSpace",
  pattern: /\s+/,
  line_breaks: true,
});
const QuotedAttribute = createToken({
  name: "QuotedAttribute",
  pattern: /"[^"]*"|'[^']*'/,
  line_breaks: true,
});
const AttributeText = createToken({
  name: "AttributeText",
  pattern: /[^\s\]="']+/,
});
const AttributeEquals = createToken({ name: "AttributeEquals", pattern: /=/ });
const UnclosedAttributeQuote = createToken({
  name: "UnclosedAttributeQuote",
  pattern: /["']/,
});
...
const CodeText = createToken({
  name: "CodeText",
  pattern: /(?:[^[]|\[(?!\/code\s*\]))+/i,
  line_breaks: true,
});
const NoParseEnd = createToken({
  name: "NoParseEnd",
  pattern: /\[\/noparse\s*\]/i,
  line_breaks: true,
  pop_mode: true,
});
const NoParseText = createToken({
  name: "NoParseText",
  pattern: /(?:[^[]|\[(?!\/noparse\s*\]))+/i,
  line_breaks: true,
});
...
class SteamBbcodeGrammar extends CstParser {
  constructor() {
    ...
    this.tag = this.RULE("tag", () => {
      ...
      const ownsBody = this.ACTION(() => {
        ...
        return (
          findSteamTagDefinition(name) !== undefined ||
          (this.lastClosingOffset.get(name) ?? -1) > opening.startOffset
        );
      });
    ...
    this.horizontalRule = this.RULE("horizontalRule", () => {
      ...
      this.OPTION({
        GATE: () =>
          tokenMatcher(this.LA(1), ClosingTag) &&
          this.LA(1).image.slice(2, -1).trim().toLowerCase() === "hr",
        DEF: () => this.CONSUME(ClosingTag),
      });
      this.ACTION(() => {
        this.depth -= 1;
      });
  ...
  countNode() {
    this.nodeCount += 1;
    if (this.nodeCount > this.limits.maxNodeCount)
      throw new SteamResourceLimitError(
        "STEAM_MAX_NODE_COUNT_EXCEEDED",
        "Source element count exceeds maxNodeCount.",
      );
  }
  ...
  enterTag() {
    ...
    if (this.depth > this.limits.maxNestingDepth)
      throw new SteamResourceLimitError(
        ...
        "Construct nesting exceeds maxNestingDepth.",
      );
  }
}
...
class SteamNamedAttributeGrammar extends CstParser {
  constructor() {
    ...
    this.namedAttributes = this.RULE("namedAttributes", () => {
      this.MANY_SEP({
        SEP: AttributeSpace,
        DEF: () => this.SUBRULE(this.assignment),
      });
    });
    /** @type {() => NativeCstNode} */
...
export function parseSteamNamedAttributes(rawAttributes) {
  ...
  return Object.freeze(
    (cst.children["assignment"] ?? []).map((entry) => {
      if (!("children" in entry))
        throw new Error("A native attribute assignment must be a CST node.");
      ...
      const tokens = values.map((value) => {
        if (!("image" in value))
          throw new Error("A native attribute value must be a token.");
        return value;
      });
      ...
      return Object.freeze({
        name: name.image,
        value: quoted ? value.slice(1, -1) : value,
      });
    }),
  );
}
...
export function parseSteamBbcodeSyntax(source, resourceLimits = {}) {
  if (typeof source !== "string")
    throw new TypeError("Steam BBCode source must be a string.");
  const limits = resolveSteamParseResourceLimits(resourceLimits);
  if (
    source.length > limits.maxInputBytes ||
    encoder.encode(source).byteLength > limits.maxInputBytes
  ) {
    return {
      source,
      cst: undefined,
      ...
      diagnostics: [
        {
          ...
          message: "UTF-8 input size exceeds maxInputBytes.",
        },
      ],
    };
  }
  const lexed = lexer.tokenize(source);
  const attributeError = checkAttributeLimit(
    source,
    lexed.tokens,
  ...
  for (const token of lexed.tokens) {
    if (token.tokenType === ClosingTag)
      parser.lastClosingOffset.set(
        token.image.slice(2, -1).trim().toLowerCase(),
        token.startOffset,
      );
  }
  ...
  return {
    ...
    children: Object.freeze(
      (cst?.children["entry"] ?? []).map((entry) =>
        projectSyntax(entry, source),
      ),
    ),
    diagnostics: [
      ...lexed.errors.map(
        (error) =>
          /** @satisfies {SteamSyntaxDiagnostic} */ ({
            code: "STEAM_LEXICAL_ERROR",
...
function checkAttributeLimit(source, tokens, maximum) {
  ...
  for (const token of tokens) {
    if (
      [TagStart, CodeStart, NoParseStart, HorizontalRuleStart].includes(
        token.tokenType,
      )
    )
      start = (token.endOffset ?? token.startOffset) + 1;
    if (
      start !== undefined &&
      [HeaderEnd, CodeHeaderEnd, NoParseHeaderEnd].includes(token.tokenType)
    ) {
      if (
        encoder.encode(source.slice(start, token.startOffset)).byteLength >
        maximum
      )
        return {
          code: "STEAM_MAX_ATTRIBUTE_BYTES_EXCEEDED",
          message: "UTF-8 attribute size exceeds maxAttributeBytes.",
        };
      start = undefined;
    }
  }
  if (
    start !== undefined &&
    encoder.encode(source.slice(start)).byteLength > maximum
  )
    return {
      code: "STEAM_MAX_ATTRIBUTE_BYTES_EXCEEDED",
      message: "UTF-8 attribute size exceeds maxAttributeBytes.",
...
function inspectStructure(cst, source) {
  ...
  while (pending.length > 0) {
    ...
    if (!node) break;
    if (node.name === "tagHeaderTail" && !node.children["HeaderEnd"])
      add(
        "STEAM_UNCLOSED_TAG_HEADER",
        "An opening tag has no closing bracket.",
        node.location,
      );
    if (node.children["UnclosedAttributeQuote"])
      add(
        "STEAM_UNCLOSED_ATTRIBUTE_QUOTE",
        "A tag attribute contains an unclosed quote.",
        node.location,
      );
    const opening =
      firstToken(node, "TagStart") ??
      firstToken(node, "CodeStart") ??
      firstToken(node, "NoParseStart");
    if (opening) {
      const closing =
        firstToken(node, "ClosingTag") ??
        firstToken(node, "CodeEnd") ??
        firstToken(node, "NoParseEnd");
      ...
      if (!closing) {
        if (findSteamTagDefinition(opening.image.slice(1).toLowerCase()))
          add(
            "STEAM_UNCLOSED_TAG",
            `Opening ${opening.image} has no closing tag.`,
            node.location,
          );
      } else if (
        opening.image.slice(1).toLowerCase() !==
        closing.image.slice(2, -1).trim().toLowerCase()
      )
        add(
          "STEAM_MISMATCHED_CLOSING_TAG",
          `${opening.image} is paired with ${closing.image}.`,
          node.location,
        );
    }
      ...
      for (const child of children) {
        if ("children" in child) pending.push(child);
          ...
          add(
            ...
            `No opening tag precedes ${child.image}.`,
            child,
          );
      }
    }
...
function unistSourceSpan(source, coordinates) {
  ...
  const { startOffset, endOffset, startLine, endLine, startColumn, endColumn } =
    coordinates;
  if (
    endOffset === undefined ||
    startLine === undefined ||
    ...
    return undefined;
  if (
    ![startOffset, endOffset, startLine, endLine, startColumn, endColumn].every(
      Number.isFinite,
    ) ||
    startOffset < 0 ||
  ...
  return {
    start: { line: startLine, column: startColumn, offset: startOffset },
      ...
      line: endLine + (endsLine ? 1 : 0),
      column: endsLine ? 1 : endColumn + 1,
      offset: endOffset + 1,
    },
  };
}
...
function projectSyntax(entry, source) {
  if ("image" in entry) {
    ...
    if (entry.tokenType === ItemBoundary)
      return Object.freeze({ type: "steamListItemBoundary", ...fields });
    if (entry.tokenType === ClosingTag)
      return Object.freeze({
        type: "steamUnmatchedClosingTag",
        tagName: entry.image.slice(2, -1).trim().toLowerCase(),
        ...fields,
      });
  ...
  const opening =
    firstToken(entry, "TagStart") ??
    firstToken(entry, "CodeStart") ??
    firstToken(entry, "NoParseStart") ??
    firstToken(entry, "HorizontalRuleStart");
  const headerTail = entry.children["tagHeaderTail"]?.[0];
  if (!opening || !headerTail || !("children" in headerTail))
    throw new Error("Native tag has no header.");
  const headerEnd = firstToken(headerTail, "HeaderEnd");
  const closing =
    firstToken(entry, "ClosingTag") ??
    firstToken(entry, "CodeEnd") ??
    firstToken(entry, "NoParseEnd");
  const fields = syntaxSource(source, entry.location);
  const header = {
    ...fields,
    rawAttributes: source.slice(
      (opening.endOffset ?? opening.startOffset) + 1,
      headerEnd?.startOffset ?? fields.sourceSpan.end.offset,
    ),
    headerClosed: headerEnd !== undefined,
    ...(closing
      ? { closingTagName: closing.image.slice(2, -1).trim().toLowerCase() }
      : {}),
  };
  if (entry.name === "code" || entry.name === "noParse") {
    return Object.freeze({
      type: "steamOpaqueTag",
      tagName: entry.name === "code" ? "code" : "noparse",
      value:
        firstToken(entry, entry.name === "code" ? "CodeText" : "NoParseText")
          ?.image ?? "",
      ...header,
    });
  }
  return Object.freeze({
    type: "steamTag",
    tagName: opening.image.slice(1).toLowerCase(),
    children: Object.freeze(
      (entry.children["element"] ?? []).map((child) =>
        projectSyntax(child, source),
      ),
    ),
    ...header,
  });
...
function syntaxSource(source, coordinates) {
  ...
  if (!span)
    throw new Error("A nonempty native syntax element has no source range.");
  return {
    rawSource: source.slice(span.start.offset, span.end.offset),
