...
export function gfmToSteamCommunityBbcode(source, options = {}) {
  if (typeof source !== "string")
    throw new TypeError("GFM source must be a string.");
  if (!options || typeof options !== "object" || Array.isArray(options))
    throw new TypeError("GFM conversion options must be an object.");
  for (const key of Object.keys(options)) {
    if (key !== "resourceLimits")
      throw new TypeError(`Unknown GFM conversion option: ${key}`);
  }
  const limits = resolveGfmConversionResourceLimits(
    options.resourceLimits === undefined ? {} : options.resourceLimits,
  );
      ...
      diagnostics: [
        {
          ...
          message:
            "GFM conversion exceeded a resource limit; no partial target was emitted.",
        },
      ],
      unsupportedSourceNodes: [],
      coverage: {
  ...
  if (Buffer.byteLength(source, "utf8") > limits.maxInputBytes)
    return failure("GFM_MAX_INPUT_BYTES_EXCEEDED");
  ...
  while (pending.length) {
    ...
    if (++nodeCount > limits.maxNodeCount)
      return failure("GFM_MAX_NODE_COUNT_EXCEEDED");
    if (entry.depth > limits.maxNestingDepth)
      return failure("GFM_MAX_NESTING_DEPTH_EXCEEDED");
  ...
  function preserve(node) {
    unsupportedSourceNodes.push({
      scope: "gfm-node",
      nodeType: node.type,
      fidelity: "unsupported",
      severity: "warning",
      code: "GFM_NODE_UNSUPPORTED_PRESERVED",
      message: `GFM ${node.type} is outside the supported Steam subset; its source is retained as literal text.`,
      ...(node.position ? { sourceSpan: node.position } : {}),
    });
    const pending = [node];
    while (pending.length) {
    ...
    if (start === undefined || end === undefined)
      throw new Error("Native GFM syntax node is missing source offsets.");
  ...
  function renderNode(node) {
    switch (node.type) {
      ...
      case "text":
        record(node, "equivalent");
        return encodeSteamLiteral(node.value);
      case "heading": {
      ...
      case "blockquote":
        record(node, "equivalent");
        return `[quote]${node.children.map(render).join("\n\n")}[/quote]`;
      case "thematicBreak":
        record(node, "equivalent");
        return "[hr][/hr]";
      case "list": {
      ...
      case "listItem":
        ...
        return "[*]" + node.children.map(render).join("\n\n");
      case "code":
        if (node.lang || node.meta || /\[\/code\s*\]/iu.test(node.value))
          return preserve(node);
        record(node, "equivalent");
      ...
      case "image": {
        ...
        if (
          ...
          !isAllowedResourceUrl(node.url, node.type) ||
          /[[\]"']/u.test(node.url) ||
          node.title ||
        ...
        return node.type === "link"
          ? `[url="${node.url}"]${node.children.map(render).join("")}[/url]`
          : `[img]${node.url}[/img]`;
      }
      case "table":
        if (
          node.align?.some((align) => align !== null) ||
          node.children.some(
            (row) => row.children.length !== node.children[0]?.children.length,
          )
        )
          return preserve(node);
        record(node, "equivalent");
  ...
  function render(node) {
    ...
    if (Buffer.byteLength(value, "utf8") > limits.maxOutputBytes)
  ...
  try {
    ...
    if (error === outputLimitExceeded)
      return failure("GFM_MAX_OUTPUT_BYTES_EXCEEDED");
    throw error;
  }
  return {
    value,
    diagnostics: unsupportedSourceNodes,
    unsupportedSourceNodes,
    coverage: {
      direction: "gfm-to-steam",
      registryVersion: steamRegistryVersion,
