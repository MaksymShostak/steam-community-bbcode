...
} from "../steam/parse-steam-bbcode.js";
import {
  findSteamTagDefinition,
  findSteamRendererDefinition,
} from "../steam/construct-definition.js";
import {
  steamRegistryVersion,
  steamConstructDefinitions,
} from "../steam/construct-registry.js";
...
import {
  steamYoutubePreviewAttributes,
  steamVideoAttributes,
} from "./steam-media-attributes.js";
...
export function steamCommunityBbcodeToMdast(source, options = {}) {
  ...
  const diagnostics = parsed.diagnostics.map((diagnostic) => ({
    ...diagnostic,
    scope: "input",
    severity: diagnostic.code.startsWith("STEAM_MAX_") ? "error" : "warning",
  ...
  function record(node, fidelity) {
    ...
    constructs.push({
      constructId: definition?.id ?? "steam.bbcode.unknown",
      fidelity,
  ...
  function preserve(node, code, message) {
    ...
    return {
      ...
      position: copyPosition(node.sourceSpan),
    };
  }
  ...
  function phrasing(node, insideLink = false) {
    ...
    if (!nodeDefinition(node))
      return preserve(
        ...
        "Unrecognized Steam source is preserved literally.",
      );
    if (
      node.type === "steamTag" &&
      node.headerClosed &&
      node.closingTagName === node.tagName &&
      (node.tagName === "previewimg" || node.tagName === "screenshot")
    ) {
      const attributes = steamPreviewImageAttributes(
        node.tagName,
        node.rawAttributes,
      );
      const definition = nodeDefinition(node);
      if (
        attributes &&
        definition &&
        node.children.every((child) => child.type === "steamText")
      ) {
        record(node, "exact");
        return {
          ...
          ...attributes,
          alt: node.children.map((child) => child.rawSource).join(""),
    ...
    if (
      ...
      node.headerClosed &&
      node.closingTagName === node.tagName &&
      (node.tagName === "url" || node.tagName === "img")
    ) {
      if (node.tagName === "url" && insideLink)
        return preserve(
          node,
          "STEAM_NESTED_LINK_PRESERVED",
          "A GFM link label cannot contain another active link.",
        );
      const kind = node.tagName === "url" ? "link" : "image";
      const body = node.children.every((child) => child.type === "steamText")
        ? node.children.map((child) => child.rawSource).join("")
        : undefined;
      const destination =
        node.rawAttributes.trim() === ""
          ? body
          : kind === "link"
            ? primaryTagAttribute(node.rawAttributes)
            : undefined;
      if (destination === undefined)
        return preserve(
          node,
          "STEAM_CONSTRUCT_PRESERVED",
          "This resource needs one unambiguous destination.",
        );
      if (destination.includes("{STEAM_CLAN_IMAGE}")) {
        const definition = findSteamRendererDefinition(
          "clan-image-placeholder",
        );
        if (!definition)
          throw new Error("Missing clan-image registry definition.");
        constructs.push({
          constructId: definition.id,
          fidelity: "unsupported",
          sourceSpan: node.sourceSpan,
        ...
        diagnostics.push({
          ...
          sourceSpan: node.sourceSpan,
          severity: "warning",
          code: "STEAM_CLAN_IMAGE_UNRESOLVED",
          fidelity: "unsupported",
          message:
            "A clan-image placeholder cannot become an active resource URL without a qualified resolution context.",
        });
        return preserve(
          node,
          "STEAM_CONSTRUCT_PRESERVED",
          "The resource retains its unresolved Steam placeholder as literal source.",
        );
      }
      if (!isAllowedResourceUrl(destination, kind))
        return preserve(
          node,
          "STEAM_UNSAFE_URL_PRESERVED",
          "The resource destination is not allowed; source is retained without an active URL.",
        );
      record(node, kind === "image" ? "equivalent" : "exact");
      const position = copyPosition(node.sourceSpan);
      return kind === "image"
        ? { type: "image", url: destination, alt: "", position }
        : {
            type: "link",
            url: destination,
            children: phrasingChildren(node.children, true),
            position,
          };
    }
    if (
      node.type === "steamOpaqueTag" &&
      node.tagName === "noparse" &&
      validUnattributedPair(node)
    ) {
      record(node, "exact");
      return {
        type: "steamNoParse",
    ...
    if (
      ...
      if (color !== undefined || node.rawAttributes.trim() === "") {
        ...
        return {
          ...
          children: phrasingChildren(node.children, insideLink),
          position: copyPosition(node.sourceSpan),
        };
      }
    }
    if (node.type === "steamTag" && validUnattributedPair(node)) {
      ...
      if (node.tagName === "u" || node.tagName === "spoiler") {
        record(node, "exact");
        return {
          type: node.tagName === "u" ? "steamUnderline" : "steamSpoiler",
          children: phrasingChildren(node.children, insideLink),
          position: copyPosition(node.sourceSpan),
        };
      }
    ...
    return preserve(
      node,
      "STEAM_CONSTRUCT_PRESERVED",
      "This construct is preserved literally in this content context.",
    );
  }
  ...
  function phrasingChildren(nodes, insideLink = false) {
    return nodes.flatMap((node) => {
      if (node.type !== "steamText" || insideLink)
        return [phrasing(node, insideLink)];
      for (const diagnostic of steamRendererTextDiagnostics(node)) {
        diagnostics.push(diagnostic);
        if (diagnostic.scope === "construct" && diagnostic.sourceSpan)
          constructs.push({
            constructId: diagnostic.constructId,
      ...
      for (const child of children) {
        if (
          child.type === "link" &&
          child.data?.steamUrlWidget &&
          child.position
        ) {
          constructs.push({
            constructId: child.data.steamUrlWidget.constructId,
            fidelity: "exact",
            sourceSpan: child.position,
          });
        }
      }
      return children;
  ...
  function list(node) {
    ...
    for (const child of node.children) {
      if (child.type === "steamListItemBoundary")
        items.push({ marker: child, body: [] });
      else {
        const last = items.at(-1);
        if (last) last.body.push(child);
        else if (
          child.type !== "steamText" ||
          !/^[ \t\r\n]*$/u.test(child.value)
        )
          return undefined;
      }
  ...
  function block(node) {
    ...
    if (
      ...
      return {
        ...
        author: separator < 0 ? attribution : attribution.slice(0, separator),
        ...(separator < 0
          ? {}
          : { steamCommentId: attribution.slice(separator + 1) }),
        children: flow(node.children),
        position: copyPosition(node.sourceSpan),
      };
    }
    if (
    ...
    if (node.tagName === "previewyoutube") {
      ...
      if (
        ...
        !definition ||
        !node.children.every(
          (child) => child.type === "steamText" && child.value.trim() === "",
        )
      )
        return undefined;
      record(node, "exact");
    ...
    if (node.tagName === "table") {
      ...
      if (!rows)
        return {
          ...
          children: [
            preserve(
              node,
              "STEAM_TABLE_STRUCTURE_PRESERVED",
              "This table or its attributes cannot be represented as a rectangular GFM table without changing its meaning.",
            ),
          ],
        };
      record(node, "exact");
      return {
        ...
        ...(layout && Object.keys(layout).length
          ? { data: { steamTableLayout: layout } }
          : {}),
        children: rows.map(({ row, cells }) => {
          record(row, "exact");
          return {
            type: "tableRow",
            position: copyPosition(row.sourceSpan),
            children: cells.map((cell) => {
              record(cell, "exact");
              return {
                type: "tableCell",
                children: phrasingChildren(cell.children),
                position: copyPosition(cell.sourceSpan),
              };
            }),
          };
        }),
      ...
      return (
        converted ?? {
          ...
          children: [
            preserve(
              node,
              "STEAM_LIST_CONTENT_PRESERVED",
              "A list needs item boundaries and cannot silently discard leading prose.",
            ),
          ],
        }
      );
    ...
    switch (node.tagName) {
      ...
      case "h3":
        ...
        return {
          type: "heading",
          depth: node.tagName === "h1" ? 1 : node.tagName === "h2" ? 2 : 3,
          children: phrasingChildren(node.children),
          position,
        };
      case "p":
        record(node, "exact");
  ...
  function flow(nodes, withinListItem = false) {
    ...
    function flush(endsAtFlowBoundary = false) {
      ...
      if (endsAtFlowBoundary)
        trimLayoutEdge(inline.length - 1, -1, /[ \t\r\n]+$/u);
      ...
      if (converted) {
        ...
        if (node.type === "steamText") {
          for (const child of inlineNodes)
            if (child.type === "text") ordinaryTextNodes.add(child);
        }
      ...
      ? [
          {
            ...
            reason: definition.contextReason,
          },
        ]
      : [],
  );
  return {
    value: { type: "root", children: flow(parsed.children) },
    diagnostics,
    coverage: {
      registryVersion: steamRegistryVersion,
...
function conversionSource(source, options) {
  if (typeof source === "string")
    return parseSteamCommunityBbcode(source, options);
  if (!isIssuedSteamBbcodeParseResult(source))
    throw new TypeError(
      "Prepared source must be a result issued by parseSteamCommunityBbcode.",
    );
  if (!options || typeof options !== "object" || Array.isArray(options))
    throw new TypeError("Conversion options must be an object.");
  for (const key of Object.keys(options)) {
    if (key !== "profile" && key !== "resourceLimits")
      throw new TypeError(`Unknown conversion option: ${key}`);
  }
  if ("resourceLimits" in options)
    throw new TypeError(
      "Resource limits must be supplied when parsing the source string.",
    );
  if (options.profile !== undefined && options.profile !== source.profile)
    throw new RangeError("Prepared source profile cannot be changed.");
  return source;
}

/** @param {SourceNode} node */
function nodeDefinition(node) {
  return node.type === "steamListItemBoundary"
    ? findSteamTagDefinition("*")
    : "tagName" in node
      ? findSteamTagDefinition(node.tagName)
      : undefined;
}
