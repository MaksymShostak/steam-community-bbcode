...
export function steamRendererTextDiagnostics(node) {
  ...
  for (const match of node.value.matchAll(
    /([:ː])[A-Za-z_][A-Za-z0-9_]*\1|\{STEAM_CLAN_IMAGE\}\//gu,
  )) {
    const clanImage = match[0].startsWith("{");
    const definition = findSteamRendererDefinition(
      clanImage ? "clan-image-placeholder" : "emoticon-expansion",
    );
    if (!definition)
      throw new Error("Missing renderer-token registry definition.");
    consume(match.index);
    ...
    diagnostics.push({
      ...
      code: clanImage
        ? "STEAM_CLAN_IMAGE_UNRESOLVED"
        : "STEAM_EMOTICON_PRESERVED_AS_TEXT",
      message: clanImage
        ? "The Steam clan-image placeholder has no qualified resolution context; its source is retained."
