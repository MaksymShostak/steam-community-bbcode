...
function steamUrlWidgetKind(value) {
  ...
  if (
    ...
    (host === "youtu.be" && /^\/[A-Za-z0-9_-]+$/u.test(url.pathname))
  )
    return "youtube-widget";
  if (
    host === "store.steampowered.com" &&
    /^\/app\/\d+(?:\/|$)/u.test(url.pathname)
  )
    return "store-widget";
  if (host === "steamcommunity.com") {
    if (
      /^\/sharedfiles\/filedetails\/?$/u.test(url.pathname) &&
      /^\d+$/u.test(url.searchParams.get("id") ?? "")
    )
      return "ugc-widget";
    if (
      /^\/(?:id|profiles)\/[^/]+\/inventory\/?$/u.test(url.pathname) &&
      /^#\d+_\d+_\d+$/u.test(url.hash)
    )
      return "inventory-widget";
  }
  if (host === "vimeo.com" && /^\/\d+\/?$/u.test(url.pathname))
    return "vimeo-widget";
  if (host === "sketchfab.com" && /^\/3d-models\/[^/]+\/?$/u.test(url.pathname))
...
export function steamTextWithUrlWidgets(node) {
  ...
  for (const match of scanner.match(node.value) ?? []) {
    ...
    children.push({
      type: "link",
      url: match.raw,
      children: [{ type: "text", value: match.raw, position }],
      position,
  ...
  if (cursor < node.value.length || children.length === 0) {
    ...
    children.push({
      type: "text",
      value,
      position: consume(node.value.length),
    });
  }
  return children;
}
