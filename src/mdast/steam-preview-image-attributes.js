...
export function steamPreviewImageAttributes(tagName, rawAttributes) {
  ...
  if (!steamImageId || !/^\d+$/u.test(steamImageId) || !details)
    return undefined;
  if (tagName === "screenshot") {
    if (fields.length !== 2 || !isAllowedResourceUrl(details, "image"))
      return undefined;
  ...
  const size =
    rawSize === "sizeThumb"
      ...
      : rawSize === "sizeFull"
        ? "full"
        : rawSize === "sizeOriginal"
          ? "original"
          : undefined;
  const alignment =
    rawAlignment === "floatLeft"
      ? "left"
      : rawAlignment === "floatRight"
        ? "right"
        : rawAlignment === "inline"
          ? "inline"
          : undefined;
  ...
  return {
    image: { kind: "guideImage", steamImageId, fileName },
    size,
    alignment,
  };
}
