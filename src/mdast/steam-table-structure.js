...
export function rectangularSteamTableRows(table) {
  ...
  for (const row of table.children) {
    ...
    if (row.type !== "steamTag" || row.tagName !== "tr" || !plainPair(row))
      return undefined;
    /** @type {Tag[]} */
    const cells = [];
    for (const cell of row.children) {
      ...
      if (
        cell.type !== "steamTag" ||
        cell.tagName !== (rows.length === 0 ? "th" : "td") ||
        !plainPair(cell)
      )
        return undefined;
    ...
    if (
      !cells.length ||
      (rows.length > 0 && cells.length !== rows[0]?.cells.length)
    )
      return undefined;
    rows.push({ row, cells });
  }
